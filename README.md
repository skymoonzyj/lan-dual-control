...
它会临时启动 Windows 被控端和 Mac 控制端页面，自动确认 Windows 画面与输入确认；Mac 控制端文本剪贴板入口可在页面中手动验证。
