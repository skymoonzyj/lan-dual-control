...
`--preflightOnly --boardSummary` 或 `--dryRun --json` 在已有 Windows host 时会同步给出 ready 后可运行的 `check-mac-client-formal-status --sendCall` 命令；如果使用备用 Agent Link Board，可加 `--server <url>`，包装器会把该地址传给内部 formal checklist 和输出的 `sendCall` 命令。
