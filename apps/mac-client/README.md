...
- 显示 Windows host 的 `video_frame`；浏览器支持 WebCodecs 时会优先请求 `h264` / `annexb` 并渲染到 canvas，不支持或连续解码失败时自动请求 MJPEG/JPEG 兜底；收到 `video_frame.timestamp` 时，视频状态和会话诊断会显示帧到达年龄或时钟偏差。
...
- H.264 当前使用浏览器 WebCodecs 解码；不支持 WebCodecs 的浏览器会自动请求 MJPEG/JPEG。真实 Windows host H.264 链路还需要 Windows 端 ffmpeg-h264 推出后继续强校验。
...
会话诊断面板已纳入页面级自检：连接成功并出现首帧后，脚本会断言“首帧”和“视频流”指标已从等待状态更新，并在对端提供 `video_frame.timestamp` 时断言视频状态和诊断行显示“到达 <ms>”或“时钟偏差”；视频表面可以是 JPEG `<img>` 或 H.264 `<canvas>`，自检会统一识别；临时 Windows host 也会断言 runtime 里显示 PID 和测试 build id，音频验收时也会断言音频诊断显示已接收帧；自检末尾会点击“断开”，确认连接状态、视频表面、音频状态和诊断指标回到干净初始态。
