...
- 文件发送按钮仅在已连接且已选文件、且 Windows host 未明确报告文件剪贴板不可用时可用，发送中和等待对端确认时会禁用；如果对端明确报告文件剪贴板不可用，页面会本地拦截文件/压缩包发送，不发出 `clipboard_file_offer`、分块或完成消息，并提示检查 Windows 文件剪贴板能力；如果对端返回失败结果（例如 `LAN011`），或 45 秒内没有返回 `clipboard_file_result`，页面会保留文件选择、显示失败/超时原因，并把按钮切换为“重新发送”。
...
- 显示 `input_ack`、视频帧和连接日志，并可一键导出或复制当前连接状态、画质、重连倒计时/原因、远端 runtime、音视频/剪贴板摘要、文件发送建议和最近事件记录；导出/复制文本不包含连接密码，文件发送失败、确认超时、等待确认或对端文件剪贴板不可用时会提示点击“重新发送”或让 Windows 端检查连接、文件剪贴板能力、权限和磁盘空间。
