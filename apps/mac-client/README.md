...
- 认证失败时显示远端返回的剩余尝试次数，清理远程画面、远端摘要、会话诊断和远端运行信息，并自动释放连接按钮，方便改密码后重连。
- 意外断线后最多自动重连 3 次，并会在等待重连时清理上一帧远程画面和远端运行信息，远端摘要显示“连接中断”；手动断开和认证失败不会自动重连。
- 手动断开会停止剪贴板监听、取消正在发送的文件、关闭音频播放、清理上一帧远程画面，并把远端摘要、会话诊断和远端运行信息重置为未就绪状态。
...
会话诊断面板已纳入页面级自检：连接成功并出现首帧后，脚本会断言“首帧”和“视频流”指标已从等待状态更新，临时 Windows host 也会断言 runtime 里显示 PID 和测试 build id，音频验收时也会断言音频诊断显示已接收帧；自检末尾会点击“断开”，确认连接状态、视频表面和诊断指标回到干净初始态。
