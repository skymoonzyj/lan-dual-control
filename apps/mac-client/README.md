...
- 密码框下方会固定提示“在这里输入 Windows 临时密码；不要发到通讯板”，并显示安全状态：空密码提示真机需临时密码，演示密码只标为本机 mock 可用，其他输入只显示“已输入”；最近连接和复制/导出诊断都不会保存或输出连接密码。
