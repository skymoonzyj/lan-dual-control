...
- 支持 H.264 流式解码入口：当前窗口环境支持 WebCodecs 时会优先请求 `h264`，收到 `annexb-base64` 帧后用 `VideoDecoder` 渲染到视频画布；会依次探测显式 Annex B 和浏览器默认 H.264 配置，解析 Annex B/AVC NAL 类型识别 IDR/SPS/PPS 关键帧，重配置后会等待关键帧再喂给解码器，连续失败时自动请求 JPEG 兜底；本机解码队列超过 8 帧或最旧帧超过 450ms 时会清掉旧队列，delta 帧等待下一关键帧，避免继续播放过期画面。
