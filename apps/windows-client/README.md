...
- Mac 提醒区和复制/导出诊断会单独识别 Mac 通过证据：当联络板或 watcher 文本里出现 `MacHostMedia 通过 passed=... media=ok`、`MacFormalLocalSmoke 通过 H.264/PCM/input-log` 等正向结果时，会显示“证据：Mac 媒体基线已通过 / Mac 本机短验收已通过”或“值守证据 ...”；这些内容不会塞进“风险”摘要，也不会触发任何 Mac 脚本、认证、密码或 input/inject。
