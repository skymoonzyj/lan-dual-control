...
- Mac 提醒区和复制/导出诊断会识别 Mac heartbeat watcher 入口：`MacHeartbeatOnce=` / `MacHeartbeatWatch=` 搭配 stale、blocked、非空 warning/blocker 或 Codex 重连风险时，会显示“Mac 单次心跳上板命令已提供”和“Mac 持续心跳 watcher 命令已提供”；干净命令清单不会误弹。
