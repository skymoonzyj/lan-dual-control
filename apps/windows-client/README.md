...
- W8 桌面原生视频长跑摘要 `W8NativeVideo=` 会单独上板，包含 `ui=html-shell`、`mainSurface=native-hwnd|native-pending`、`canvasRole=diagnostic-fallback`、`presenting=yes|no`、`presentGap=<decoded-presentFrames>`、`queueDrops=<n>`、`queueDropScope=predecode|queue` 和 `queueReason=<reason>`；真实 Mac H.264 长跑时，优先用它判断 MF/D3D11 已解码的帧是否真的进入 Windows 原生窗口 Present。`mainSurface=native-hwnd` 表示 UI 仍是 HTML 壳，但视频主面已有原生 HWND Present 证据；`queueDropScope=predecode` 表示旧 native queue 预过滤层在丢旧保实时，不等于 native decoder/present 没收到帧。
- W8 原生低延迟队列现在会阻止已拒绝的旧 H.264 delta 继续进入持久 MF/D3D11 decoder：`need-keyframe` / `waiting-keyframe` 只更新队列丢帧和等待关键帧诊断，`submittedFrames` 不再因为这些旧帧增长；下一条恢复关键帧到达后才继续推进原生 decoder 和 Present。
