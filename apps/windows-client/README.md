...
- Mac 提醒区和复制/导出诊断会消费干净的通用正向 `Evidence=` / `evidence=` 字段：例如 `Evidence=MacClientPageOnline,MacClientDiagnosticsOk,MacHostMediaOk` 会显示“Mac client 页面在线 / Mac client 诊断已通过 / Mac 媒体基线已通过”。只有无 failed/stale/offline、无非空 warning/blocker 且不包含 Mac 脚本命令的片段才会被识别；`MacHeartbeat=blocked ... evidence=正在重新连接 5/5` 这类风险证据不会误判为健康证据。
