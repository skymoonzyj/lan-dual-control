...
- 控制端针对真实体验 blocker 做了稳定性防护：底部状态栏固定单行省略，避免连接后声音/剪贴板/帧率状态文字换行导致远程画面上下抖动；PCM 播放调度加入 80ms 低水位预缓冲，队列超过 450ms 时只修剪尚未开始播放的未来音源，保留当前正在播放的片段，并用 120ms 重同步预缓冲接上最新帧；从后台/切出窗口恢复可见时，如果 WebAudio 队列或计划 source 已明显堆积，会清掉旧 source 并重新接到当前重同步缓冲；恢复后的短跟随窗口内如果队列再次堆到约 180ms，会用 `queue-overflow-snap-live` 丢旧并贴回实时附近，同时记录重同步次数、可见恢复次数和最近原因；音频状态文字限频，降低突发音频帧造成的断续。页面自检会覆盖 `Audio buffer guards` 和 `Live status layout stability`。
