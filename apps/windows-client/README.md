...
- W8 桌面原生视频长跑摘要 `W8NativeVideo=` 会单独上板，包含 `presenting=yes|no` 和 `presentGap=<decoded-presentFrames>`；真实 Mac H.264 长跑时，优先用它判断 MF/D3D11 已解码的帧是否真的进入 Windows 原生窗口 Present。
