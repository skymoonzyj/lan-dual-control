...
- Mac 提醒区和复制/导出诊断会消费稳定 `MacPowerHealth=` / `MacUnattendedHealth=` 字段：`system-sleep-enabled`、`display-sleep-enabled` 会显示为“系统睡眠未关闭 / 显示器睡眠未关闭”，`launch-agent-not-loaded` 会显示为“自启动未加载”；`ok warnings=none blockers=none` 和单独的 `MacPowerPlan=` 预览命令不会误弹风险。Windows 端只做本地中文提示和诊断导出，不运行 `pmset`、不加载 LaunchAgent、不运行 Mac 脚本、不认证、不发送密码/input/inject。
