...
- 复制/导出诊断新增现场视频和现场声音统计：快速摘要会输出实收 FPS、请求/协商 Hz、平均/最大帧间隔、视频帧数、解码队列、本机队列毫秒、解码延迟、本地过期丢帧、跳过 delta、需要关键帧和最近原因；声音会输出当前 WebAudio 队列毫秒、80/70/450 ms 缓冲阈值、接收/播放/丢弃计数、重同步次数和最近原因。用户反馈“卡”“不像 60Hz”或“声音断续”时，先复制诊断即可看到关键现场数据。
...
- 支持 H.264 流式解码入口：当前窗口环境支持 WebCodecs 时会优先请求 `h264`，收到 `annexb-base64` 帧后用 `VideoDecoder` 渲染到视频画布；会依次探测显式 Annex B 和浏览器默认 H.264 配置，解析 Annex B/AVC NAL 类型识别 IDR/SPS/PPS 关键帧，重配置后会等待关键帧再喂给解码器；首个 decoded surface 画出前会给 1080p/60Hz 等真实 H.264 解码一个短暖机宽限，避免初始队列短暂超过 8 帧/450ms 就关 decoder 后长期等待关键帧；首帧渲染后，本机解码队列超过 8 帧或最旧帧超过 450ms 仍会清掉旧队列，delta 帧先按协商刷新率等待下一关键帧，超时也保持 H.264 并重启视频流，不再因关键帧等待把 Mac 拉到后台 JPEG；只有连续解码失败或显式兜底路径才请求 JPEG。
