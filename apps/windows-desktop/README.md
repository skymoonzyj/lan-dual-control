...
当前策略是：低延迟正常帧保留；积压时若有较新关键帧，丢旧并跳到该关键帧；没有可用关键帧时清掉 delta 积压并等待关键帧，避免旧帧继续堆积。下一步把 worker 线程里的 BGRA8 present texture 接到真实 HWND/native renderer 并真正 Present，同时补 NV12 shader 转换，逐步替换 WebCodecs/canvas 作为最终体验路径。
