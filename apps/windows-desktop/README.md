...
当前策略是：低延迟正常帧保留；积压时若有较新关键帧，丢旧并跳到该关键帧；没有可用关键帧时清掉 delta 积压并等待关键帧，避免旧帧继续堆积。当前 BGRA8 和 NV12 decoded sample 都已能走到真实 HWND swapchain `Present`；下一步补 stream-change、窗口 resize/swapchain resize、device-lost 恢复和真实 Mac 长跑验证，逐步替换 WebCodecs/canvas 作为最终体验路径。
