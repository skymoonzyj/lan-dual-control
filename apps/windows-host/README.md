...
需要确认所有带 `Help` switch 的 Windows PowerShell 入口 `-Help` / `-h` 都只打印帮助、不误启动 host、watcher、Agent Link、不采集 WASAPI、浏览器探针或发送输入时，可以运行统一覆盖自检；当前会覆盖 `test-windows-host.ps1`、`dev-lab.ps1`、`test-windows-client-browser.ps1` 和 `check-webcodecs-h264-support.ps1` 等 17 个入口、34 条命令。需要给 Agent Link Board 时直接加 `--boardSummary`，输出一行无密摘要：
