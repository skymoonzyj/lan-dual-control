...
需要把 Windows 体检结论同步到 Agent Link Board 时，可以加 `--boardSummary`。它会输出一行无密摘要，并在 host 在线时透出 Mac 端下一步可运行的 readiness / formal checklist 命令；`--json` 也会带 `boardSummary`、`macClientReadinessCommands`、`windowsHostMediaReadinessPowerShellCommand`、`windowsReverseControlGrantCommand`、`windowsReverseControlGrantPowerShellCommand`、`windowsVideoEncoderSupportCommand`、`windowsVideoEncoderSupportPowerShellCommand`、`windowsWgcBenchmarkCommand` 和 `windowsWgcBenchmarkPowerShellCommand` 字段，方便自动化脚本消费。若运行中的 Windows host 已打开一次性临时反控授权，或刚安全拒绝过 Mac 的反控请求，runtime 摘要会显示 `reverse=temporary-grant` / `reverse=pending-request`，`boardSummary` 也会沿用同一状态词；默认需确认策略下还会保留 `ReverseGrant=` 和 `ReverseGrantPs=`，同时保留 `WindowsHostMediaPs=`、`WindowsVideoSupport=`、`WindowsVideoSupportPs=`、`WindowsWgcBenchmark=` 和 `WindowsWgcBenchmarkPs=` 基础刷新率/码率 benchmark 入口，即使运行时摘要被压缩也不会丢。若 Mac 端已经通过通讯板发起 Windows host 验收 call，再加 `--checkBoard` 会只读读取 Agent Link Board `/api/state.currentCall`，并在 JSON / 普通输出 / boardSummary 中提示 active Mac -> Windows call；DONE call 不会当作待办：
...
需要快速比较 WGC H.264 raw-bgra 与 NV12 两条内部源格式时，使用 `scripts/windows/compare-windows-wgc-h264-sources.mjs`，或在现场 PowerShell 里用 `scripts/windows/compare-windows-wgc-h264-sources.ps1`。它只是包装现有 benchmark，默认顺序跑 raw-bgra 和 NV12，并输出普通摘要、`--json` / `-Json` 或可发 Agent Link Board 的 `--boardSummary` / `-BoardSummary`；不会连接 Mac、不会认证正式密码、不会发送输入或执行 `inject`。`check-windows-resume-status --boardSummary` 也会给出 `WindowsWgcBenchmark=` / `WindowsWgcBenchmarkPs=` 和 `WindowsWgcCompare=` / `WindowsWgcComparePs=` 安全命令。日常收口可以用 `check-windows-host-readiness --probeWgcH264Sources` 跑一组更短的 30Hz/10Mbps readiness 探针。
