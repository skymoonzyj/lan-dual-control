...
需要确认所有带 `Help` switch 的 Windows PowerShell 入口 `-Help` / `-h` 都只打印帮助、不误启动 host、watcher、Agent Link、不采集 WASAPI 或发送输入时，可以运行统一覆盖自检；当前会覆盖 `test-windows-host.ps1` 和 `dev-lab.ps1` 等 14 个入口、28 条命令：

```powershell
node E:\codex\lan-dual-control\scripts\windows\test-windows-powershell-help.mjs
node E:\codex\lan-dual-control\scripts\windows\test-windows-powershell-help.mjs --shell pwsh
```
