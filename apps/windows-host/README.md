...
- `hello`、`auth_request`、`session_offer`、`display_settings`、`input_event`、`input_ack`、`clipboard_text` 和 `reverse_control_request` 消息处理；未认证连接会被拒绝，同一连接内密码错误 3 次后会关闭连接。反控请求默认按 `LAN008` 安全拒绝并说明需要用户确认，并把最近一次被拒绝的请求暴露到 `/discovery.capabilities.reverseControlGrant.lastRequest`，方便 Windows 本机面板提示“刚收到请求”；Windows 本机桌面壳可打开 30 秒一次性临时授权窗口，下一次请求会通过并立即消耗；只有显式设置实验策略 `LAN_DUAL_WINDOWS_REVERSE_CONTROL_MODE=accept` 时才会长期自动同意，适合可信局域网短测。
...
需要把当前 Windows host 在线状态同步到 Agent Link Board，或让 Mac 端直接跑真连前 readiness / formal checklist，可以用无密摘要输出。它会给出 `check-mac-client-readiness`、`check-mac-client-formal-status`、统一识别用的 `MacClientFormalChecklist=check-mac-client-formal-status --boardSummary`、ready 后可用的 `check-mac-client-formal-status --sendCall`，用于随机运行期密码不可取回时现场重启并双端本地输入临时密码的 `WindowsSecureAuthPath=`，以及本机 `WindowsHostMedia=check-windows-host-readiness --checkBoard --probeMedia --boardSummary` / `WindowsHostMediaPs=check-windows-host-readiness.ps1 -CheckBoard -ProbeMedia -BoardSummary` 媒体基线命令、`WindowsVideoSupport=check-windows-video-encoder-support --boardSummary` / `WindowsVideoSupportPs=check-windows-video-encoder-support.ps1 -BoardSummary` 视频能力体检命令、`WindowsWgcSupport=check-windows-wgc-support --boardSummary` / `WindowsWgcSupportPs=check-windows-wgc-support.ps1 -BoardSummary` WGC/WinRT/GPU 专项预检命令、`WindowsWebCodecs=check-webcodecs-h264-support --requireCodec avc1.42C02A --boardSummary` / `WindowsWebCodecsPs=check-webcodecs-h264-support.ps1 -RequireCodec avc1.42C02A -BoardSummary` 浏览器 H.264 解码预检命令、`WindowsWgcBenchmark=benchmark-windows-wgc-settings --profile 60:20000:balanced --durationMs 1800 --boardSummary` / `WindowsWgcBenchmarkPs=benchmark-windows-wgc-settings.ps1 -Profile 60:20000:balanced -DurationMs 1800 -BoardSummary` 基础刷新率/码率 benchmark 命令，以及 `WindowsWgcCompare=compare-windows-wgc-h264-sources --profile 60:20000:balanced --durationMs 1800 --boardSummary` / `WindowsWgcComparePs=compare-windows-wgc-h264-sources.ps1 -Profile 60:20000:balanced -DurationMs 1800 -BoardSummary` raw-bgra vs NV12 对照命令；默认安全拒绝反控时，摘要也会给出 `WindowsReverseGrantStatus=`、`WindowsOpenOneTimeReverseGrant=`、Node fallback，以及兼容旧入口的 `ReverseGrant=` / `ReverseGrantPs=`。加 `--checkBoard` / `-CheckBoard` 时，摘要还会提示 active Mac -> Windows `currentCall`，DONE call 不会当作待办。状态入口不会启动服务、不会认证、不会打印密码：
...
需要把 Windows 音频能力同步到 Agent Link Board 时，可以输出一行无密摘要；它只列 WASAPI/DirectShow 能力和探针状态，默认不采集声音：

```powershell
node E:\codex\lan-dual-control\scripts\windows\check-windows-audio-devices.mjs --boardSummary
```
...
需要把 Windows 体检结论同步到 Agent Link Board 时，可以加 `--boardSummary`。它会输出一行无密摘要，并在 host 在线时透出 Mac 端下一步可运行的 readiness / formal checklist 命令；`--json` 也会带 `boardSummary`、`macClientReadinessCommands`、`windowsSecureAuthPath`、`windowsFirewallStatusCommand`、`windowsFirewallPreviewCommand`、`windowsHostMediaReadinessPowerShellCommand`、`windowsReverseControlGrantCommand`、`windowsReverseControlGrantPowerShellCommand`、`windowsVideoEncoderSupportCommand`、`windowsVideoEncoderSupportPowerShellCommand`、`windowsWgcSupportCommand`、`windowsWgcSupportPowerShellCommand`、`windowsWebCodecsH264Command`、`windowsWebCodecsH264PowerShellCommand`、`windowsWgcBenchmarkCommand`、`windowsWgcBenchmarkPowerShellCommand`、`windowsWgcCompareCommand` 和 `windowsWgcComparePowerShellCommand` 字段，方便自动化脚本消费。若运行中的 Windows host 已打开一次性临时反控授权，或刚安全拒绝过 Mac 的反控请求，runtime 摘要会显示 `reverse=temporary-grant` / `reverse=pending-request`，`boardSummary` 也会沿用同一状态词；默认需确认策略下还会保留 `ReverseGrant=` 和 `ReverseGrantPs=`，同时保留 `WindowsSecureAuthPath=`、`WindowsFirewallStatus=`、`WindowsFirewallPreview=`、`WindowsHostMediaPs=`、`WindowsVideoSupport=`、`WindowsVideoSupportPs=`、`WindowsWgcSupport=`、`WindowsWgcSupportPs=`、`WindowsWebCodecs=`、`WindowsWebCodecsPs=`、`WindowsWgcBenchmark=`、`WindowsWgcBenchmarkPs=`、`WindowsWgcCompare=` 和 `WindowsWgcComparePs=` WGC/H.264 相关入口，即使运行时摘要被压缩也不会丢。`WindowsFirewallStatus=` 只做 JSON 状态检查，`WindowsFirewallPreview=` 只做 `--dryRunRule --ruleProfile Private` 预览，不会自动添加防火墙规则。若 Mac 端已经通过通讯板发起 Windows host 验收 call，再加 `--checkBoard` 会只读读取 Agent Link Board `/api/state.currentCall`，并在 JSON / 普通输出 / boardSummary 中提示 active Mac -> Windows call；DONE call 不会当作待办：
...
需要确认所有带 `Help` switch 的 Windows PowerShell 入口 `-Help` / `-h` 都只打印帮助、不误启动 host、watcher、Agent Link、不采集 WASAPI、浏览器探针或发送输入时，可以运行统一覆盖自检；当前会覆盖 `test-windows-host.ps1`、`dev-lab.ps1`、`test-windows-client-browser.ps1`、`check-webcodecs-h264-support.ps1`、`check-windows-wgc-support.ps1`、`benchmark-windows-wgc-settings.ps1` 和 `compare-windows-wgc-h264-sources.ps1` 等 20 个入口、40 条命令。需要给 Agent Link Board 时直接加 `--boardSummary`，输出一行无密摘要：

```powershell
node E:\codex\lan-dual-control\scripts\windows\test-windows-powershell-help.mjs --timeoutMs 10000 --boardSummary
node E:\codex\lan-dual-control\scripts\windows\test-windows-powershell-help.mjs --shell pwsh --timeoutMs 10000 --boardSummary
```
...
node E:\codex\lan-dual-control\scripts\windows\check-windows-wgc-support.mjs --boardSummary
powershell.exe -NoProfile -ExecutionPolicy Bypass -File E:\codex\lan-dual-control\scripts\windows\check-windows-wgc-support.ps1 -BoardSummary
powershell.exe -NoProfile -ExecutionPolicy Bypass -File E:\codex\lan-dual-control\scripts\windows\check-windows-wgc-support.ps1 -Json
```

需要发到 Agent Link Board 时用 `--boardSummary` / `-BoardSummary`，只输出一行无密摘要；`--json` / `-Json` 也会带同一份 `boardSummary` 字段。默认一键体检只把 WGC 预检作为信息项；等真正实现 WGC 采集后，再用 `--requireWgc` 把它变成强校验。
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File E:\codex\lan-dual-control\scripts\windows\test-windows-host.ps1 -Help
...
一键反控请求的安全策略可单独跑专项回归。它会启动临时 Windows host，覆盖未认证 `LAN002`、默认 `deny` 下的 `LAN008` 和最近请求状态、一次性临时授权消耗、缺少 `requestId`、显式 `accept` 实验策略和 `disabled` 发现能力；不使用正式密码、不发送输入、不执行 `inject`：
...
如果只想预览 Private 入站规则而不改系统，可以加 dry-run：

```powershell
node E:\codex\lan-dual-control\scripts\windows\check-windows-firewall.mjs --host 0.0.0.0 --port 43770 --dryRunRule --ruleProfile Private
```
...
普通输出模式下，矩阵外层会默认每 10 秒报告当前 case、attempt、临时 host/client/debug 端口和剩余子进程超时，同时把 `--progressIntervalMs` 透传给每个页面自检，让内层连接/视频/音频等待也继续输出页面快照；现场想更快看进度可加 `--progressIntervalMs 5000`，传 `0` 可关闭。`--json` 仍保持纯机器输出，不混入进度行；需要直接同步到 Agent Link Board 时可加 `--boardSummary`，脚本会输出一行无密摘要，包含通过数、失败 case、重试次数和 no input/inject 安全说明。
...
WGC 参数基准脚本位于 `scripts/windows/benchmark-windows-wgc-settings.mjs`，现场 PowerShell 等价入口是 `scripts/windows/benchmark-windows-wgc-settings.ps1`。它会构建或复用本地 Rust helper，顺序启动临时 Windows host，并用 `observe-windows-host-video --screenMode wgc --resourceSampleTree true --json` 跑多档刷新率/码率对照；可输出普通摘要、`--json` / `-Json`，或直接发 Agent Link Board 的 `--boardSummary` / `-BoardSummary` 单行无密摘要。`2026-06-14 20:35` 本机 `1280x720`、每档 2.2 秒短基准已确认 30/60/120Hz 都能协商到对应会话刷新率，但当前 WGC 仍是 `FrameArrived` 事件驱动，静态桌面实收约 9-12 FPS：30Hz/10M 为 21 帧、9.25 FPS、平均约 78 KB；60Hz/20M 为 25 帧、11.11 FPS、平均约 83 KB；120Hz/40M sharp 为 27 帧、12.22 FPS、平均约 121 KB。`LAN_DUAL_WINDOWS_WGC_REPEAT_LAST_FRAME=1` 或观察/基准脚本 `--wgcRepeatLastFrame true` / `--repeatLastFrame` / PowerShell `-RepeatLastFrame` 可启用重复最后一帧诊断模式，host 会在没有新 WGC 帧时复用上一帧，并在 `video_frame` 带 `repeatedFrame`、`sourceTimestamp`、`contentAgeMs`。默认 `LAN_DUAL_WINDOWS_WGC_REPEAT_LAST_FRAME_MODE=full` 保持旧行为，会重发完整 JPEG；显式设为 `signal` 或脚本加 `--wgcRepeatLastFrameMode signal` / `--repeatLastFrameMode signal` / `-RepeatLastFrameMode signal` 时，重复帧只发 `repeatPreviousFrame=true`、`payloadBytes=0` 和尺寸/时间戳诊断，不再重发 base64 图片。`2026-06-14 20:55` full repeat 短基准显示 30Hz/10M 可到 56 帧、30.45 FPS，60Hz/20M 到 62 帧、33.92 FPS，120Hz/40M sharp 到 68 帧、37.78 FPS，内容年龄最大约 80-96 ms；`2026-06-14 21:20` signal repeat 的 60Hz/20M 两轮短基准约 31-32 FPS、重复信令帧 33-35、平均图片 payload 约 17-23 KB、内容年龄最大 79-82 ms。signal 模式主要降低重复帧带宽和 JSON/base64 解析压力，不会生成更多真实源帧；后续仍应推进 WebSocket 二进制帧、H.264/硬编和 Mac client 真连观感验收。

需要快速比较 WGC H.264 raw-bgra 与 NV12 两条内部源格式时，使用 `scripts/windows/compare-windows-wgc-h264-sources.mjs`，或在现场 PowerShell 里用 `scripts/windows/compare-windows-wgc-h264-sources.ps1`。它只是包装现有 benchmark，默认顺序跑 raw-bgra 和 NV12，并输出普通摘要、`--json` / `-Json` 或可发 Agent Link Board 的 `--boardSummary` / `-BoardSummary`；不会连接 Mac、不会认证正式密码、不会发送输入或执行 `inject`。`check-windows-resume-status --boardSummary`、`start-windows-host --status --boardSummary` 和 `check-windows-host-readiness --boardSummary` 都会先给出 `WindowsWgcSupport=` / `WindowsWgcSupportPs=` 专项预检和 `WindowsWebCodecs=` / `WindowsWebCodecsPs=` 浏览器 H.264 解码预检，再给出 `WindowsWgcBenchmark=` / `WindowsWgcBenchmarkPs=` 和 `WindowsWgcCompare=` / `WindowsWgcComparePs=` 安全命令。日常收口可以用 `check-windows-host-readiness --probeWgcH264Sources` 跑一组更短的 30Hz/10Mbps readiness 探针。
...
node E:\codex\lan-dual-control\scripts\windows\benchmark-windows-wgc-settings.mjs --profile 60:20000:balanced --durationMs 1800 --boardSummary
powershell.exe -NoProfile -ExecutionPolicy Bypass -File E:\codex\lan-dual-control\scripts\windows\benchmark-windows-wgc-settings.ps1 -Profile 60:20000:balanced -DurationMs 1800 -BoardSummary
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File E:\codex\lan-dual-control\scripts\windows\compare-windows-wgc-h264-sources.ps1 -Profile 60:20000:balanced -DurationMs 1800 -BoardSummary
```
...
调 H.264 前可以先探测当前浏览器 WebCodecs 对各个 `avc1.*` 的支持，避免把浏览器启动参数问题误判成编码器问题。需要发通讯板时用 `--boardSummary` / `-BoardSummary`，PowerShell 包装入口适合现场直接复制；探针只打开本机临时浏览器配置和 loopback 页面，不启动 Windows host、不认证、不采集屏幕/声音、不发送 input/inject：

```powershell
node E:\codex\lan-dual-control\scripts\windows\check-webcodecs-h264-support.mjs --requireCodec avc1.42C02A
node E:\codex\lan-dual-control\scripts\windows\check-webcodecs-h264-support.mjs --requireCodec avc1.42C02A --boardSummary
powershell.exe -NoProfile -ExecutionPolicy Bypass -File E:\codex\lan-dual-control\scripts\windows\check-webcodecs-h264-support.ps1 -RequireCodec avc1.42C02A -BoardSummary
```
