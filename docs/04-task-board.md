...
- Windows 端新增只读视频编码能力体检 `scripts/windows/check-windows-video-encoder-support.mjs`，可一次汇总 FFmpeg H.264 软件/硬件编码器、WGC 预检和浏览器 WebCodecs H.264 支持，并支持 JSON/强校验。本机 `2026-06-15` 强校验通过，发现 `h264_nvenc`、`h264_qsv`、`h264_amf`、`h264_mf`、`h264_d3d12va` 和 `libx264`。同日已完成显式 WGC helper JPEG -> FFmpeg/NVENC H.264 桥接原型、raw-bgra -> FFmpeg/NVENC H.264 正确性原型、raw-bgra helper 内部 `binary-frame-v1`，以及 NV12 -> FFmpeg/NVENC H.264 bridge。当前 NV12 真实 helper + `h264_nvenc` 在 `1280x720` 下 2.2 秒收到 67 帧、约 30.06 FPS，pipeline=`windows-wgc-helper-nv12-ffmpeg-h264`，`helperProtocol=binary-frame-v1`；下一步应推进 helper 原生硬编或 Mac client 真连资源/观感对照。
