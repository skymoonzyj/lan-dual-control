...
状态：进行中。
- [ ] W2-BACKGROUND-VISIBILITY-VIDEO-FREEZE：W2/W3 首屏可见画面 gate 已过后，用户切出 Codex/控制端窗口时视频卡死但声音正常。证据：连接仍在 `192.168.31.122:43770`，视频 H.264/annexb-base64/真实屏幕，约 `62 FPS`，到达约 `308ms`，状态回到等待关键帧，`queue=470ms`，本地过期丢帧 `142`，原因 `queue-overflow-wait-keyframe`；音频接收/播放 `9583/9583`。owner：Windows 控制端，需查 page visibility/background throttling、`requestAnimationFrame`/canvas draw、WebCodecs queue/backpressure、`visibilitychange` 恢复策略。Mac 只保持 host 在线；不请求密码、不发 input/inject。
