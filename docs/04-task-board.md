...
- [x] Windows 恢复总览消费 Mac heartbeat watcher 入口：`check-windows-resume-status --checkBoard` 会从联络板最近摘要安全提取 `MacHeartbeatOnce=watch-mac-heartbeat --once --sendStatus --boardSummary` 和 `MacHeartbeatWatch=watch-mac-heartbeat --sendStatus --intervalMs 30000`，写入 JSON、普通输出和 `--boardSummary`；Windows 控制端诊断在风险上下文中会显示“Mac 单次心跳上板命令已提供”“Mac 持续心跳 watcher 命令已提供”。
