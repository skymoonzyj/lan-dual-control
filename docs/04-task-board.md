...
- [x] Windows 控制端把干净 `MacHeartbeatFreshness=fresh` 显示为值守证据：只要稳定字段 fresh 且同片段没有 failed/stale/offline、非空 warning/blocker 或 Mac 脚本命令，就会进入“Mac 心跳正常”；stale 仍走风险摘要。不运行 Mac 脚本、不认证、不请求或发送密码、不发 input/inject。
