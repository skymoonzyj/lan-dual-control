...
- [x] Windows 侧联络板 Mac 提醒 watcher 可后台运行：优先 PowerShell 7，Mac 授权/权限/502/blocked/长时间无更新，以及 Mac 发给 Windows 的 active currentCall 会弹本机提醒；支持状态/停止/重启和防重复启动，并有无弹窗自动回归。
- [x] 上传到 GitHub 仓库。
- [x] Mac mini 到位后克隆仓库。
