...
- [x] Windows WGC H.264 raw-bgra vs NV12 对照支持 PowerShell 包装入口：`compare-windows-wgc-h264-sources.ps1 -Profile 60:20000:balanced -DurationMs 1800 -BoardSummary/-Json` 可输出一行无密摘要或机器可读 JSON；`check-windows-resume-status` 现会给出 `WindowsWgcCompare=` / `WindowsWgcComparePs=`，便于卡顿/帧率排查时从恢复总览直接进入较重的源格式对照。
...
- [x] Windows PowerShell 入口建立统一 `-Help/-h` 纯帮助覆盖自检：`test-windows-powershell-help.mjs` 自动发现带 `Help` switch 的 `.ps1`，当前覆盖 19 个入口、38 条命令，包含 `test-windows-host.ps1`、`dev-lab.ps1`、`test-windows-client-browser.ps1`、`check-webcodecs-h264-support.ps1`、`check-windows-wgc-support.ps1` 和 `compare-windows-wgc-h264-sources.ps1`；已锁定帮助路径不启动 host/watcher/Agent Link、不改机器环境、不初始化 WASAPI/采集声音、不认证、不发送密码/Token/input/inject。
- [x] Windows PowerShell help 自检支持 `--boardSummary`：可一行输出 Windows PowerShell 或 PowerShell 7 的 38/38 覆盖摘要，`--json` 同步带 `boardSummary` 字段；`check-windows-resume-status` 的 JSON/普通输出/`--boardSummary` 也会给出 `PowerShellHelp=` 和 `PowerShellHelpPwsh=` 两条无密命令。
