...
- Mac 端新增 `scripts/mac/check-mac-formal-local-smoke.mjs` 正式端到端前本机短验收聚合脚本：复用 H.264、PCM 和 input-log 探针，默认要求正式密码来自 `LAN_DUAL_PASSWORD` 或 `--promptPassword`，拒绝空密码和 `demo-password`；`--promptPassword` 会先响铃再弹 macOS 隐藏密码对话框，密码只经环境变量传给子进程；不会启动 host、不会打印密码、不会执行 `inject`。`scripts/mac/test-mac-formal-local-smoke.mjs` 用临时假 Mac host 覆盖密码安全和三探针聚合成功。
- Mac 端新增 `scripts/mac/check-mac-host-readiness.mjs` 一键体检聚合脚本：默认只读检查 Node/Swift、Mac host build、直接启动输入默认值、启动助手语法/干跑、键盘映射覆盖和 `/discovery` 状态；可加 `--probeVideo --probeAudio --probeInputLog --probeStartHelper` 串联真实 H.264、PCM、log 输入和启动助手临时端口自测。正式密码 host 做深度探针时可加 `--promptPassword`，它会先响铃再弹 macOS 隐藏密码对话框，readiness 会通过环境变量传给子探针，不放进命令参数。Mac host 最新源码已用 `IOHIDCheckAccess` 只读探测真实 Input Monitoring 状态，不再把 `/discovery.permissions.inputMonitoring` 硬编码为 `false`；readiness 也会默认提示运行中 host build 是否落后当前 git，并列出旧 build 后变动的 Mac host runtime 源码文件，支持 `--requireInputMonitoring` 单独强制输入监控权限。常用部署验收可用 `--profile deploy`，深度本机部署验收可用 `--profile deep`。
...
- Windows 端新增只读视频编码能力体检 `scripts/windows/check-windows-video-encoder-support.mjs`，可一次汇总 FFmpeg H.264 软件/硬件编码器、WGC 预检和浏览器 WebCodecs H.264 支持，并支持 JSON/强校验。本机 `2026-06-15` 强校验通过，发现 `h264_nvenc`、`h264_qsv`、`h264_amf`、`h264_mf`、`h264_d3d12va` 和 `libx264`。同日已完成显式 WGC helper JPEG -> FFmpeg/NVENC H.264 桥接原型：真实 WGC helper + `h264_nvenc` + repeat full 短观察 3 秒 91 帧、约 30.29 FPS、最大间隔 59ms；随后完成 raw-bgra -> FFmpeg/NVENC H.264 正确性原型，并进一步把 raw-bgra helper 内部承载从 JSON/base64 切到 `binary-frame-v1`。当前真实 helper + `h264_nvenc` 在 `1280x720` 下两轮 2.2 秒短测均收到 66 帧、约 29.6-30.0 FPS，pipeline=`windows-wgc-helper-raw-bgra-ffmpeg-h264`，`helperProtocol=binary-frame-v1`；静态桌面复验 fresh helper frame 2、repeat full 64。下一步应推进 NV12 或 helper 原生硬编。
...
- [ ] Windows WGC 二进制 raw 管道、NV12 或 helper 原生硬编，以及 Mac client 真连观感验收。
- [ ] 安装包。
- [ ] 开机自启。
