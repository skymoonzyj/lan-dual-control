...
- [x] W3 Windows WebAudio 重同步短预缓冲：队列超过 450ms flush 旧 source 后，下一帧使用 120ms 重同步预缓冲，普通 underrun 仍保持 80ms；复制诊断“现场声音”显示 `缓冲 80/70/450/120 ms`、重同步次数和 `queue-overflow-flush-old`。`test-windows-client-browser --diagnosticsOnly` 先红于 20.08s 旧行为，再绿于 120ms 重同步路径。不改协议、不认证、不请求或发送密码、不发 input/inject。
...
- [x] Windows N2 WebAudio 队列治理：当播放队列超过 450ms 时停止旧的已排队音源、重置 120ms 重同步预缓冲并播放最新 PCM 帧，记录 `audioResyncCount` / `audioLastDropReason=queue-overflow-flush-old`，复制诊断“现场声音”同步导出重同步次数和原因。`test-windows-client-browser --diagnosticsOnly` 覆盖 overflow flush old queue。只做本地模拟/diagnosticsOnly，不改系统声音、不认证、不请求密码、不发 input/inject。
