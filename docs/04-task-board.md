...
- Windows 侧已新增 `scripts/windows/test-windows-script-help.mjs`，可统一回归 `scripts/windows/*.mjs` 的 `--help` / `-h` 纯帮助入口；当前覆盖 22 个脚本、44 条帮助命令，并修正旧脚本 `-h` 会被忽略的问题。
...
- [ ] Windows WGC helper 连续帧 pacing、资源/码率长测和 Mac client 真连观感验收。
- [ ] 安装包。
- [ ] 开机自启。
