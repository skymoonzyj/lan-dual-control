...
- 当前屏幕采集默认优先 FFmpeg gdigrab 持续 MJPEG，PowerShell/System.Drawing 系统截图作为兜底，全部失败时会回退模拟帧；Windows 被控端也已新增可选 `ffmpeg-h264` 过渡模式，使用 FFmpeg `libx264` 输出 `video_frame.codec=h264`、`encoding=annexb-base64`、`capturePipeline=windows-ffmpeg-gdigrab-h264` 和 `codecString`，真实桌面权限下本机 720p/30Hz 短基线为 73 帧/2.5 秒、约 28.83 FPS、最大间隔 53ms，普通沙盒上下文可能因 FFmpeg `gdigrab error 5` 回退 mock，主要用于 Mac client H.264 接收链路对接。WGC 目前已完成支持预检，当前 Windows 11 build `26200`、WGC WinRT 类型、`GraphicsCaptureSession.IsSupported()` 和硬件 GPU 检测均通过；本轮已新增 `LAN_DUAL_WINDOWS_WGC_HELPER` JSON 行 helper 接入点，配置 helper 后可走 `windows-wgc-helper-jpeg` 管线，临时 mock helper 已验证出帧，但真正原生 WGC helper 尚未完成。音频默认未配置时继续模拟，可显式设置 `LAN_DUAL_WINDOWS_AUDIO_MODE=wasapi` 试用系统声音 loopback，或配置 `LAN_DUAL_WINDOWS_AUDIO_DEVICE` 试用 DirectShow PCM；后续仍需实现原生 Windows Graphics Capture helper，输入注入后续重点是真实 `system` 模式手感和安全边界验收。
