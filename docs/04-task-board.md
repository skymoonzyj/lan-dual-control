...
- [x] Windows host 根目录双击启动入口：新增 `Start-Windows-Host.cmd`，默认调用 `start-windows-host.ps1 -PromptPassword -RequirePassword`，让用户在 Windows 本机终端隐藏输入临时密码启动被控端；`-Help` 只打印说明不启动 host。`test-windows-host-start-helper` 覆盖入口存在、帮助快速退出、调用隐藏密码参数且不包含明文密码/环境密码参数。不改协议、不认证、不发送密码、不发 input/inject。
