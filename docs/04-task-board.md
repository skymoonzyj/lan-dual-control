...
- [x] W2 Windows H.264 JPEG fallback 冷却恢复：`keyframe-wait-timeout-fallback` 请求 MJPEG/JPEG 保画面后，Windows 控制端会等待 2.5 秒且收到 3 帧稳定 JPEG，再用现有 `display_settings` 尝试恢复 `preferredVideoCodec=h264` / `preferredVideoEncoding=annexb`；页面自测红绿灯通过。不改协议、不认证、不请求或发送密码、不发 input/inject。- [x] W3/M1 Windows 控制端消费 `MacRemoteAudioStatus=`：Mac 提醒/值守文本里的 `local-playback-active`、`localOutput=audible`、`remoteOnly=not-active` 会被翻译成“Mac 本机仍会出声 / 远端独占声音未开启 / 远端独占声音需用户明确同意 / 不会自动改 Mac 音量”风险；页面自测红绿灯通过。不改协议、不认证、不请求或发送密码、不发 input/inject。
