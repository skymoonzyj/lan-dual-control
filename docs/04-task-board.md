...
状态：进行中。
- [x] W2/W3 复测结果脱敏上板助手：新增 `post-w2w3-retest-board.mjs`，支持 `--text` / `--file` 输入真实 `W2W3Retest=`，默认 dry-run，`--send` 时先做危险标记检查，再发送 `W2W3Retest=` 并自动追加 `W2H264BoardDiagnosis=`。专项测试先红于脚本不存在，再绿于帮助、dry-run、发送两条消息、拒绝危险输入、缺少复测证据和通讯板 ok:false。不请求密码、不认证、不发真实输入事件。
