...
- Mac 端新增 `scripts/mac/observe-mac-media.mjs` 媒体聚合入口：只连接已运行的 Mac host，顺序汇总 H.264 视频和系统 PCM 音频观察，支持 `--json` 和一行无密 `--boardSummary`；单路成功单路失败时摘要标为 `Mac media baseline partial`，全部执行链路失败时保留 `failed <数量>`，`--json` 提供机器可读 `summary.status=ok|partial|failed`；不会启动 host、不会发送输入、不会执行 `inject`，密码只通过 `LAN_DUAL_PASSWORD` 传给子探针，默认不播放测试音。可选 `--resourceSample` 会只读读取本机 Mac host PID 并采样 CPU/RSS，采样不可用时只在摘要里标记 unavailable，不影响媒体观测成败。
