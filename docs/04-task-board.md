...
- Windows 被控端已新增 `scripts/windows/observe-windows-host-media.mjs`，用于顺序串联视频观察和 WASAPI 音频观察并输出统一媒体基线报告，避免并发临时 host 干扰采集；现支持 `--boardSummary` 输出一行可发 Agent Link Board 的无密媒体基线摘要，包含请求分辨率/Hz/Mbps、视频 FPS/最大间隔/帧年龄、音频稳态 FPS/最大间隔/帧年龄和资源采样状态；失败时也会输出脱敏 `Windows media: failed` 单行摘要，`--json` 失败路径仍返回可解析报告和 `boardSummary`，视频/音频其中一路失败时会继续尝试另一条链路并保留成功结果，且不回显密码或执行输入；本轮音频顺序路径通过，当前桌面会话的 FFmpeg `gdigrab` 视频路径出现 `error 5` / mock fallback，真实视频基线待桌面捕获稳定后复测或由 WGC 采集替换。
