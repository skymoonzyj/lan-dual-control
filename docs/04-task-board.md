...
- Windows 控制端已增加 Mac 主机诊断状态条：会汇总 `permissions`、`hostMode`、`capturePipeline`、`source`、`droppedFrames`、`input_ack` 和剪贴板模式，真机权限、输入拒绝或采集回退问题可直接在画面内看到。
...
- [ ] Windows 控制端把远端文件写入 Windows 系统文件剪贴板或提供安全保存 UI。
- [ ] 大文件传输速度、剩余时间和断点续传。
