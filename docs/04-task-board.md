...
- [x] Windows host 启动/状态助手和 readiness 摘要也会输出 WGC 基础 benchmark 命令：JSON 带 `windowsWgcBenchmarkCommand`，普通输出和 `--boardSummary` 带 `WindowsWgcBenchmark=benchmark-windows-wgc-settings --profile 60:20000:balanced --durationMs 1800 --boardSummary`，方便从本机被控入口直接跑刷新率/码率基础排查。
