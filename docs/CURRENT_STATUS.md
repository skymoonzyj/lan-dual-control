...
- Windows 控制端已有中文界面、局域网连接、连接历史、画质设置、缩放模式、适应窗口黑边输入防护、认证失败剩余次数提示、诊断状态条、Mac H.264 启动回退原因显示、真实 Mac H.264/WebCodecs 画布解码、真实 Mac 音频 PCM 播放、文本剪贴板、远端文件托盘、桌面版远端文件写入 Windows 系统文件剪贴板和桌面壳。
...
- Mac host 的 `/discovery` 和 `hello_ack` 会带可选 `runtime` 诊断：`processId`、`startedAt`、`uptimeSeconds` 和 `buildId`；`LAN_DUAL_BUILD_ID` 可在启动时指定，方便识别是否连到了旧进程。
