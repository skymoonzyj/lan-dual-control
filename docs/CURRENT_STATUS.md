...
- Mac 端新增 `scripts/mac/check-mac-host-readiness.mjs` 一键体检入口：默认低风险检查 Node/Swift、Mac host build、启动助手语法/干跑、键盘映射覆盖和当前 `/discovery` 状态，并汇总 `/discovery.permissions` 的屏幕录制、辅助功能和输入监控状态；可加 `--requireControlPermissions` 强制要求屏幕录制和辅助功能已开启。也可显式加 `--probeVideo --probeAudio --probeInputLog --probeStartHelper` 做 H.264、PCM、log 输入和启动助手临时端口深度验收，`--json` 可输出机器可读摘要。
