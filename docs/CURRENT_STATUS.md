...
- Windows 桌面壳远端文件写入系统文件剪贴板已升级为原生分块路径：前端每次传 1MB 到 Tauri，原生层校验偏移、最终字节数和 512MB 总量上限后写入 Windows 文件剪贴板；新建写入任务前会尽力清理 7 天以上旧临时目录，近期目录会保留以保证系统文件剪贴板可继续粘贴；旧的一次性写入命令保留作兼容回退，Rust 单测已覆盖偏移错误、写超、未写完、取消清理和超限拒绝。
