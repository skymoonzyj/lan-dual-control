...
- Windows 控制端已有中文界面、局域网连接、连接历史、画质设置、缩放模式、适应窗口黑边输入防护、认证失败剩余次数提示、诊断状态条、Mac host 运行时 PID/启动时间/build 显示、Mac H.264 启动回退原因显示、真实 Mac H.264/WebCodecs 画布解码、重配置后 H.264 关键帧等待、真实 Mac 音频 PCM 播放、文本剪贴板、远端文件托盘、桌面版远端文件分块写入 Windows 系统文件剪贴板、桌面版本机被控启动/低风险/部署/深度体检入口和桌面壳。
...
- Windows 桌面壳远端文件写入系统文件剪贴板已升级为原生分块路径：前端每次传 1MB 到 Tauri，原生层校验偏移、最终字节数和 512MB 总量上限后写入 Windows 文件剪贴板；新建写入任务前会尽力清理 7 天以上旧临时目录，近期目录会保留以保证系统文件剪贴板可继续粘贴；控制端事件日志和远端文件托盘状态会显示最近一次临时目录，工具栏可一键打开，失败后复制按钮会提示可重试写入；旧的一次性写入命令保留作兼容回退，Rust 单测已覆盖偏移错误、写超、未写完、取消清理、超限拒绝和临时目录白名单。
