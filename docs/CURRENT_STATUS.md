...
- Mac 控制 Windows 的 Web 控制端原型已新增到 `apps/mac-client`：可发现/连接 Windows host、显示 JPEG/data-url 画面、发送鼠标/键盘输入事件并显示 `input_ack`，也可手动发送文本 `clipboard_text` 并显示 `clipboard_ack`，以及选择文件后按 `clipboard_file_*` 分块发送；新增 PCM 音频播放入口，可播放 `pcm-f32le-base64` 过渡音频帧，mock 音频帧只显示状态。Windows 本机页面级自检已验证真实 `windows-ffmpeg-gdigrab-mjpeg` 画面、log 模式输入确认和文本剪贴板 `system` 确认。
