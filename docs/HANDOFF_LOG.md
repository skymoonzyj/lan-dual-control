...
- 默认 `start-mac-client --status --boardSummary` 仍只做本地页面状态检查，不读取 Agent Link Board；只有显式 `--checkBoard` 才访问 `/api/state`。
- 新字段只从 Agent Link Board 当前 status/event 的安全短字段派生；优先 `MacUnattendedHealth=`，缺失时退到 `MacPowerHealth=`，并拒绝疑似 `--password`、token、secret、pwd/passwd 的候选。
- `stale` 只提醒先刷新独立值守/电源证据，不改变页面在线/离线判定，不启动页面，不认证，不发送 call/input/inject。
修改文件：
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-client-start-helper.mjs --timeoutMs 15000` 先失败在 `--checkBoard` 路径缺 `MacUnattendedFreshness`。
- 绿灯：实现后同一专项回归通过，并覆盖默认不读板、JSON、boardSummary、`MacUnattendedHealth` 优先级、`MacPowerHealth` fallback 和敏感文本不提升。
遗留问题：
- 当前真实 `MacUnattendedFreshness=stale` 仍表示独立值守证据旧了；真正修改系统睡眠或加载 LaunchAgent 仍需用户现场确认后人工执行。
下一步建议：
- 如果只想确认本地页面是否在线，继续用默认 `start-mac-client --status --boardSummary`；如果还要同屏看值守/电源证据新旧，再加 `--checkBoard`。
是否改了协议：否。
是否需要另一端配合：暂不需要。
...
日期：2026-06-19 继续推进
开发端：Mac Codex
本轮目标：让 `MacClientDiagnostics=` / `check-mac-client-readiness` 第一屏也能提示当前 `Mac Unattended` / `MacPowerHealth` 证据是否新鲜。
完成内容：
- `check-mac-client-readiness --checkBoard --json/--boardSummary` 新增 JSON `board.macUnattendedFreshness`，并在 `MacClientDiagnostics=` 摘要和普通输出里转述 `MacUnattendedFreshness=fresh|stale checkedAgeMs=<毫秒> thresholdMs=600000 checkedAt=<时间> source=MacUnattendedHealth|MacPowerHealth`。
- 新字段只从 Agent Link Board `/api/state` 当前 status/event 的安全短字段派生；优先 `MacUnattendedHealth=`，缺失时退到 `MacPowerHealth=`，并拒绝疑似 `--password`、token、secret、pwd/passwd 的候选。
- `stale` 只提醒先刷新独立值守/电源证据，不改变 readiness blockers、Windows host probe、call、formal checklist 或反控授权命令。
- 本轮只补只读 readiness 摘要，不运行 `pmset`、不提权、不改系统、不写 plist、不加载 `launchctl`、不认证、不请求或发送密码、不发 call/input/inject。
修改文件：
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-client-readiness.mjs --timeoutMs 15000` 先失败在 help 缺 `board.macUnattendedFreshness`。
- 绿灯：实现后同一专项回归通过，并覆盖 JSON、boardSummary、普通输出、`MacUnattendedHealth` 优先级、`MacPowerHealth` fallback 和敏感文本不提升。
遗留问题：
- 当前真实 `MacUnattendedFreshness=stale` 仍表示独立值守证据旧了；真正修改系统睡眠或加载 LaunchAgent 仍需用户现场确认后人工执行。
下一步建议：
- 如果 `MacClientDiagnostics=` 看到 `MacUnattendedFreshness=stale`，先运行同屏或 heartbeat/resume/formal 里的 `MacUnattendedSendStatus=` 刷新独立值守证据，再继续 Mac client readiness / formal smoke。
是否改了协议：否。
是否需要另一端配合：暂不需要。
...
- `--boardSummary` 新增同一条 `MacLaunchAgentPlan=`，让只看最新心跳的人也能复制 LaunchAgent dry-run 规划入口。
- 测试覆盖 help、JSON commands、离线/在线/stale/power warning 摘要和安全边界：LaunchAgent plan 命令不能含 `--write`、`--force`、`launchctl`、密码、server、call、input_event 或 inject。
- 本轮只补 Mac heartbeat 的可复制只读值守加载规划入口，不运行 `pmset`、不提权、不改系统、不写 plist、不加载 `launchctl`、不认证、不请求或发送密码、不发 call/input/inject。
修改文件：
- `scripts/mac/check-mac-heartbeat.mjs`
- `scripts/mac/test-mac-heartbeat.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-heartbeat.mjs --timeoutMs 12000` 先失败在 help 缺 `macLaunchAgentPlanCommand`。
- 绿灯：实现后同一专项回归通过。
- 完整验证和推送状态见本轮提交记录。
遗留问题：
- 真实 LaunchAgent loaded 状态仍需用户现场确认后人工执行 `launchctl` 加载/打印复查，再复跑 `MacUnattendedStatus` / `MacHeartbeatOnce` 刷新证据；本轮只提供 dry-run 规划入口。
下一步建议：
- Windows 侧已能消费 `MacLaunchAgentPlan=`；后续拉取后只需确认 `MacHeartbeat=` 第一屏也能被 Windows 控制端/恢复总览正常展示或导出。
是否改了协议：否。
是否需要另一端配合：暂不需要。
...
日期：2026-06-19 继续推进
开发端：Mac Codex
本轮目标：让 `MacPowerPlan=` / `plan-mac-power-settings` 第一屏也能直接给出安全 `MacLaunchAgentPlan=` 值守加载规划入口。
完成内容：
- `plan-mac-power-settings` JSON `commands` 新增 `macLaunchAgentPlan`，指向 `node scripts/mac/install-mac-host-launch-agent.mjs --boardSummary`。
- `--boardSummary` 新增同一条 `MacLaunchAgentPlan=`，普通输出新增 “Mac LaunchAgent plan”。
- 测试覆盖 help、JSON、boardSummary、普通输出和安全边界：LaunchAgent plan 命令不能含 `--write`、`--force`、`launchctl`、密码、server、call、input_event 或 inject。
- 本轮只补 Mac power plan 的可复制只读值守加载规划入口，不运行 `pmset`、不提权、不改系统、不加载 `launchctl`、不认证、不请求或发送密码、不发 call/input/inject。
修改文件：
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-power-plan.mjs --timeoutMs 8000` 先失败在 help 缺 `commands.macLaunchAgentPlan`。
- 绿灯：实现后同一专项回归通过。
- 完整验证和推送状态见本轮提交记录。
遗留问题：
- 真实系统睡眠/显示睡眠和 LaunchAgent loaded 状态仍需用户现场确认后人工执行 `pmset` / `launchctl` 相关命令，再复跑 `MacUnattendedStatus` / `MacHeartbeatOnce` 刷新证据。
下一步建议：
- Windows 侧已能消费 `MacLaunchAgentPlan=`；后续拉取后只需按既有 diagnostics-only 确认组合摘要仍显示电源预案和值守加载预案。
是否改了协议：否。
是否需要另一端配合：暂不需要。
...
- 发现 Windows host 和未发现 Windows host 两种 `--boardSummary` 都新增 `MacPowerPlan=`。
- 普通输出新增 “Mac power settings dry-run plan”。
- 测试覆盖 help、发现成功 JSON/boardSummary/普通输出、未发现 Windows host JSON/boardSummary 和安全边界：命令不能含 `--apply`、`sudo`、密码、server、call、input_event 或 inject。
- 本轮只补 Mac Windows discovery 的可复制只读预案入口，不运行 `pmset`、不提权、不改系统、不认证、不请求或发送密码、不发 call/input/inject。
修改文件：
- `scripts/mac/discover-windows-hosts.mjs`
- `scripts/mac/test-discover-windows-hosts.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-discover-windows-hosts.mjs --timeoutMs 10000` 先失败在 help 缺 `macPowerPlanCommand`。
- 绿灯：实现后同一专项回归通过。
- 完整验证和推送状态见本轮提交记录。
遗留问题：
- 真实系统睡眠/显示睡眠仍需用户现场确认后人工执行预览里的 `pmset` 命令，再复跑 `MacUnattendedStatus` / `MacHeartbeatOnce` 刷新证据。
下一步建议：
- Windows 侧如只拿到 `MacClientDiscoverWindows=`，可消费同一稳定 `MacPowerPlan=`；协议和 Windows 端字段暂不需要改。
是否改了协议：否。
是否需要另一端配合：暂不需要。
...
- `--boardSummary` 新增同一条 `MacPowerPlan=`；普通输出新增 “Mac power settings dry-run plan”。
- 测试覆盖 help、离线 JSON、普通输出、boardSummary 和安全边界：命令不能含 `--apply`、`sudo`、密码、server、call、input_event 或 inject。
- 本轮只补 Mac client readiness 的可复制只读预案入口，不运行 `pmset`、不提权、不改系统、不认证、不请求或发送密码、不发 call/input/inject。
修改文件：
- `scripts/mac/check-mac-client-readiness.mjs`
- `scripts/mac/test-mac-client-readiness.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-client-readiness.mjs --timeoutMs 12000` 先失败在 help 缺 `commands.macPowerPlanCommand`。
- 绿灯：实现后同一专项回归通过。
- 完整验证和推送状态见本轮提交记录。
遗留问题：
- 真实系统睡眠/显示睡眠仍需用户现场确认后人工执行预览里的 `pmset` 命令，再复跑 `MacUnattendedStatus` / `MacHeartbeatOnce` 刷新证据。
下一步建议：
- Windows 侧如只拿到 `MacClientDiagnostics=`，可消费同一稳定 `MacPowerPlan=`；协议和 Windows 端字段暂不需要改。
是否改了协议：否。
是否需要另一端配合：暂不需要。
...
日期：2026-06-19 继续推进
开发端：Mac Codex
本轮目标：让 Mac client formal checklist 第一屏也能直接给出安全 `MacPowerPlan=` 电源预案入口。
完成内容：
- `check-mac-client-formal-status` JSON `runPlan.commands` 新增 `macPowerPlanCommand`，指向 `node scripts/mac/plan-mac-power-settings.mjs --profile all --sleep 0 --displaySleep 0 --networkWake on --boardSummary`。
- `--boardSummary` 新增同一条 `MacPowerPlan=`；普通 runPlan 输出新增 “Mac power settings dry-run plan”。
- 测试覆盖 help、离线 JSON、ready JSON、discover JSON、普通人工 runPlan、boardSummary 和安全边界：命令不能含 `--apply`、`sudo`、密码、server、call、input_event 或 inject。
- 本轮只补 Mac client formal checklist 的可复制只读预案入口，不运行 `pmset`、不提权、不改系统、不认证、不请求或发送密码、不发 call/input/inject。
修改文件：
- `scripts/mac/check-mac-client-formal-status.mjs`
- `scripts/mac/test-mac-client-formal-status.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-client-formal-status.mjs --timeoutMs 15000` 先失败在 help 缺 `runPlan.commands.macPowerPlanCommand`。
- 绿灯：实现后同一专项回归通过。
- 完整验证和推送状态见本轮提交记录。
遗留问题：
- 真实系统睡眠/显示睡眠仍需用户现场确认后人工执行预览里的 `pmset` 命令，再复跑 `MacUnattendedStatus` / `MacHeartbeatOnce` 刷新证据。
下一步建议：
- Windows 侧如只拿到 `MacClientFormalChecklist=`，可消费同一稳定 `MacPowerPlan=`；协议和 Windows 端字段暂不需要改。
是否改了协议：否。
是否需要另一端配合：暂不需要。
...
日期：2026-06-19 继续推进
开发端：Mac Codex
本轮目标：让 Mac host readiness 第一屏也能直接给出安全 `MacPowerPlan=` 电源预案入口。
完成内容：
- `check-mac-host-readiness` JSON `commands` 新增 `macPowerPlanCommand`，指向 `node scripts/mac/plan-mac-power-settings.mjs --profile all --sleep 0 --displaySleep 0 --networkWake on --boardSummary`。
- `--boardSummary` 新增同一条 `MacPowerPlan=`；普通输出 NEXT 新增 “Mac power settings dry-run plan”。
- 测试覆盖 help、默认 JSON、离线 media boardSummary、active currentCall boardSummary 和安全边界：命令不能含 `--apply`、`sudo`、密码、server、call、input_event 或 inject。
- 本轮只补 host readiness 的可复制只读预案入口，不运行 `pmset`、不提权、不改系统、不加载 LaunchAgent、不认证、不请求或发送密码、不发 input/inject。
修改文件：
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-host-readiness-board.mjs --timeoutMs 12000` 先失败在 help 缺 `commands.macPowerPlanCommand`。
- 绿灯：实现后同一专项回归通过。
- 完整验证和推送状态见本轮提交记录。
遗留问题：
- 真实系统睡眠/显示睡眠仍需用户现场确认后人工执行预览里的 `pmset` 命令，再复跑 `MacUnattendedStatus` / `MacHeartbeatOnce` 刷新证据。
下一步建议：
- Windows 侧如只拿到 `MacHostReadiness=`，可消费同一稳定 `MacPowerPlan=`；协议和 Windows 端字段暂不需要改。
是否改了协议：否。
是否需要另一端配合：暂不需要。
...
- `check-mac-resume-status` JSON `commands` 新增 `macPowerPlanCommand`，离线/在线 `--boardSummary` 和普通报告都新增同一条 `MacPowerPlan=` / “Mac power settings dry-run plan”。
- 测试覆盖 help、JSON、boardSummary、普通报告和安全边界：命令必须保持 dry-run，不含 `--apply`、`sudo`、密码、server、call、input_event 或 inject。
- 本轮只转述已有 `plan-mac-power-settings` 预案，不运行 `pmset`、不提权、不改系统、不加载 LaunchAgent、不认证、不请求或发送密码、不发 input/inject。
修改文件：
- `scripts/mac/check-mac-heartbeat.mjs`
- `scripts/mac/test-mac-heartbeat.mjs`
- `scripts/mac/check-mac-resume-status.mjs`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-resume-status.mjs --timeoutMs 12000` 先失败在离线摘要缺 `MacPowerPlan`；`node scripts/mac/test-mac-heartbeat.mjs --timeoutMs 12000` 先失败在 help 缺 `macPowerPlanCommand`。
- 绿灯：实现后两条回归均通过。
- 完整验证和推送状态见本轮提交记录。
遗留问题：
- 真实系统睡眠/显示睡眠仍需用户现场确认后人工执行预览里的 `pmset` 命令，再复跑 `MacUnattendedStatus` / `MacHeartbeatOnce` 刷新证据。
下一步建议：
- Windows 端如果已经消费 `MacPowerPlan=`，可以直接在 heartbeat/resume 第一屏风险摘要里提示复制；若尚未消费，也不阻塞 Mac 侧继续真实联调。
是否改了协议：否。
是否需要另一端配合：暂不需要。
...
日期：2026-06-19 继续推进
开发端：Windows Codex
本轮目标：修正 `MacHeartbeatFreshness=fresh` 的 Windows 控制端语义，避免把“摘要新鲜”误当成“心跳健康”。
完成内容：
- 按 Mac 端同步的语义，`MacHeartbeatFreshness=fresh` 现在只表示 Agent Link Board 上的心跳摘要足够新；Windows 控制端不再把它单独显示为“Mac 心跳正常”值守证据。
- 如果同一诊断里 `MacHeartbeat=status=blocked`、`reason=mac-codex-stale` 或有非空 blocker/warning，即使 `MacHeartbeatFreshness=fresh`，控制端仍按风险摘要显示，不输出“Mac 心跳正常”证据。
- `MacHeartbeatFreshness=stale` 的过旧风险和相对时间展示保留；干净 `MacHeartbeat=status=ok` 与明确 `Evidence=` / `MacEvidence=` 正向标签仍可进入值守证据。
修改文件：
- `apps/windows-client/app.js`
- `scripts/windows/test-windows-client-browser.mjs`
- `apps/windows-client/README.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/windows/test-windows-client-browser.mjs --diagnosticsOnly --timeoutMs 45000` 失败在 fresh-only 和 blocked+fresh 两个场景仍输出 `Mac 心跳正常` 证据。
- 绿灯：实现后同一命令通过。
遗留问题：
- 本轮只修正 Windows 控制端本地解析和展示；不运行 Mac 脚本、不认证、不请求或发送密码、不发 input/inject。
下一步建议：
- 后续看 `MacHeartbeatFreshness=fresh` 时只判断“摘要是否新”；要判断 Mac 是否健康，继续看 `MacHeartbeat` 的 `status/reason/blockers/warnings` 或稳定正向证据标签。
是否改了协议：否。
是否需要另一端配合：不需要。
...
日期：2026-06-19 继续推进
开发端：Windows Codex
本轮目标：记录上一轮曾误设的 `MacHeartbeatFreshness=fresh` 证据判断，并说明该判断已被上一条修正：fresh 只表示摘要新鲜，不单独代表心跳健康。
完成内容：
- `apps/windows-client/app.js` 曾新增干净 `MacHeartbeatFreshness=fresh` 证据判断；该判断已在后续修正中移除，避免语义误导。
- stale 的 `MacHeartbeatFreshness=` 仍走风险摘要；上轮已实现的“Mac 心跳摘要过旧（心跳检查 / Mac Codex / 联络板）”不受影响。
- 页面 diagnostics-only 已改为覆盖 fresh-only 不进入值守证据、blocked+fresh 不掩盖风险的路径。
修改文件：
- `apps/windows-client/app.js`
- `scripts/windows/test-windows-client-browser.mjs`
- `apps/windows-client/README.md`
...
下一步建议：
- 后续 Windows 第一屏只看到 `MacHeartbeatFreshness=fresh` 时，只把它当成“摘要够新”；健康证据继续看 `MacHeartbeat=status=ok` 或 `Evidence=` / `MacEvidence=`。
是否改了协议：否。
是否需要另一端配合：不需要。
...
- Mac 提醒区会显示“证据：Mac formal E2E 已就绪”，复制/导出诊断会写入“值守证据 Mac formal E2E 已就绪”；同段仍有 warning/blocker/stale/失败时继续按风险摘要处理。
- 页面 diagnostics-only 新增正向 `MacFormalE2E` 证据和导出报告覆盖，确认不会触发风险提示。
修改文件：
- `apps/windows-client/app.js`
- `scripts/windows/test-windows-client-browser.mjs`
- `apps/windows-client/README.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：新增测试后，`node scripts/windows/test-windows-client-browser.mjs --diagnosticsOnly --timeoutMs 45000` 先失败在 `positiveMacFormalE2eAttention.evidenceSummary` 为空。
- 绿灯：实现解析后，`node --check apps/windows-client/app.js`、`node --check scripts/windows/test-windows-client-browser.mjs`、`node scripts/windows/test-windows-client-browser.mjs --diagnosticsOnly --timeoutMs 45000` 通过。
遗留问题：
- 本轮只消费 Mac 已输出的 formal E2E 就绪证据，不主动运行 Mac formal E2E、不认证、不请求密码。
下一步建议：
- 如果下一轮最新通讯板出现 `MacFormalE2E status=ok readyToCall=true checklist=passed blockers=none warnings=none`，Windows 侧先把它当作正向 readiness 证据；只有同段出现非空 warning/blocker、stale、失败或授权问题时才继续排查。
是否改了协议：否；只增加 Windows 控制端本地解析、显示和诊断导出。
是否需要另一端配合：不需要。
...
- 当同段 Mac client page/readiness 摘要已经有离线、不可达、失败、非空 warning/blocker、Windows host、认证、联络板、视频、build 或 repo 风险上下文时，Mac 提醒区、快速摘要和复制/导出诊断会提示对应安全命令。
- 新增页面自测覆盖风险上下文与干净命令清单：`warnings=none blockers=none` 的页面状态/诊断命令不会误弹。Windows 端只提示/复制，不启动 Mac client、不运行 Mac 脚本、不认证、不发密码/input/inject。
修改文件：
- `apps/windows-client/app.js`
- `scripts/windows/test-windows-client-browser.mjs`
- `apps/windows-client/README.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/windows/test-windows-client-browser.mjs --diagnosticsOnly --timeoutMs 45000` 先失败在 `MacClientPage=` / `MacClientDiagnostics=` 风险上下文缺少对应中文命令提示。
- 绿灯：实现后复跑同一命令通过。
- 收尾复跑：`node --check apps/windows-client/app.js`、`node --check scripts/windows/test-windows-client-browser.mjs`、`node scripts/windows/test-windows-client-browser.mjs --diagnosticsOnly --timeoutMs 45000`、`git diff --check`、触碰文件冲突标记扫描。
遗留问题：
- 本轮不运行 Mac 端 `start-mac-client --status` 或 `check-mac-client-readiness`，只消费它们上板后的安全标签；真实页面状态和 readiness 仍由 Mac 端或人工显式运行。
下一步建议：
- 现场看到“Mac client 页面状态命令已提供”或“Mac client 诊断命令已提供”时，优先让 Mac 端刷新页面状态/readiness 一行摘要，再按需粘贴 Mac client 页面“复制诊断”。
是否改了协议：否；只补 Windows 控制端本地解析、页面回归和文档。
是否需要另一端配合：不需要；Mac 端已有 `MacClientPage=` / `MacClientDiagnostics=` 输出，本轮只消费这些标签。
...
- 当同段 `MacFormalE2E=` 有非空 warning/blocker、失败或 not-ready，并带 `MacClientBrowserSelfTest=` 时，Mac 提醒区、快速摘要和复制/导出诊断会显示“Mac client 本地 browser 自测命令已提供”。
- 干净 `warnings=none blockers=none` 命令清单仍不误弹；Windows 端只提示/复制，不运行 Mac 脚本、不认证、不传密码。
修改文件：
- `apps/windows-client/app.js`
- `scripts/windows/test-windows-client-browser.mjs`
- `apps/windows-client/README.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/windows/test-windows-client-browser.mjs --diagnosticsOnly --timeoutMs 45000` 先失败在 `MacFormalE2E=readyToCall=false ... MacClientBrowserSelfTest=` 缺少“Mac client 本地 browser 自测命令已提供”。
- 绿灯：实现后复跑同一命令通过。
- 收尾复跑：`node --check apps/windows-client/app.js`、`node --check scripts/windows/test-windows-client-browser.mjs`、`node scripts/windows/test-windows-client-browser.mjs --diagnosticsOnly --timeoutMs 45000`、`git diff --check` 与触碰文件冲突标记扫描。
遗留问题：
- 本轮只消费 Mac 摘要标签，不启动或运行 Mac 本地 browser 自测；真实自测仍由 Mac 端在用户/自动化明确选择时运行。
下一步建议：
- Mac 端若继续给 `MacFormalE2E=`、`MacFormalLocalSmoke=` 或 heartbeat/resume 增加稳定安全命令标签，Windows 端优先按同一模式做“风险上下文才提示、干净清单不误弹”的消费回归。
是否改了协议：否；只补 Windows 控制端诊断识别和文档。
是否需要另一端配合：不需要；等待 Mac 端后续推送时再拉取对接即可。

## 2026-06-19 Mac Codex

日期：2026-06-19 继续推进
开发端：Mac Codex
本轮目标：让 `MacFormalE2E=` / formal E2E readiness 摘要也暴露本地 Mac client browser 自测入口。
完成内容：
- `check-mac-formal-e2e-status --json/--boardSummary` 新增 `commands.macClientBrowserSelfTestCommand` / `MacClientBrowserSelfTest=`。
- 新命令统一为 `node scripts/mac/test-mac-client-browser-self-test-wrapper.mjs --boardSummary`，用于真实 Windows host 认证、LAN 或授权链路暂未准备好时，先排除 Mac client 页面、反控 UI、剪贴板和 input-log 流程退化。
- 离线、在线、callText、board summary 和 `--help/-h` 均保留该入口；formal E2E readiness 本身仍不使用真实 Windows host、不请求密码、不认证、不发送 call/input_event、不执行 `inject`。
修改文件：
- `scripts/mac/check-mac-formal-e2e-status.mjs`
- `scripts/mac/test-mac-formal-e2e-status.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-formal-e2e-status.mjs --timeoutMs 30000` 先失败在 `check-mac-formal-e2e-status --help` 缺 `commands.macClientBrowserSelfTestCommand`。
- 绿灯：实现后复跑 `node scripts/mac/test-mac-formal-e2e-status.mjs --timeoutMs 30000` 通过。
- 最终收尾：`node --check scripts/mac/check-mac-formal-e2e-status.mjs`、`node --check scripts/mac/test-mac-formal-e2e-status.mjs`、`node scripts/mac/test-mac-formal-e2e-status.mjs --timeoutMs 30000`、`node scripts/mac/test-mac-script-help.mjs --script check-mac-formal-e2e-status.mjs --timeoutMs 10000 --boardSummary`、离线 JSON 样例、`git diff --check` 与触碰文件冲突标记扫描。
遗留问题：
- 当前真实 Mac host 仍是旧 build；本轮没有重启 host，也没有跑需要用户密码或真实 Windows host 的 browser smoke。
下一步建议：
- 只看到 `MacFormalE2E=` 时，如果真实 Windows host 密码/LAN/授权暂未就绪，可先复制 `MacClientBrowserSelfTest=`；用户在场且准备真实连接密码时再走已有 `MacClientPromptPasswordSmoke=` 或正式验收流程。
是否改了协议：否；只补 Mac formal E2E readiness 的安全命令标签和自测。
...
日期：2026-06-19 继续推进
开发端：Mac Codex
本轮目标：让 `MacFormalLocalSmoke=` / 本机短验收摘要也暴露本地 Mac client browser 自测入口。
完成内容：
- `check-mac-formal-local-smoke --json/--boardSummary` 新增 `commands.macClientBrowserSelfTestCommand` / `MacClientBrowserSelfTest=`。
- 新命令统一为 `node scripts/mac/test-mac-client-browser-self-test-wrapper.mjs --boardSummary`，用于无人值守或真实 Windows host 密码/LAN/授权未就绪时先排除 Mac client 页面、反控 UI、剪贴板和 input-log 流程退化。
- 成功摘要、失败摘要、普通输出和 `--help/-h` 均保留该入口；本机短验收本身仍不使用真实 Windows host、不认证、不发送 call/input_event、不执行 `inject`。
修改文件：
- `scripts/mac/check-mac-formal-local-smoke.mjs`
- `scripts/mac/test-mac-formal-local-smoke.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-formal-local-smoke.mjs --timeoutMs 30000` 先失败在 `check-mac-formal-local-smoke --help` 缺 `commands.macClientBrowserSelfTestCommand`。
- 绿灯：实现后复跑 `node scripts/mac/test-mac-formal-local-smoke.mjs --timeoutMs 30000` 通过。
- 最终收尾：`node --check scripts/mac/check-mac-formal-local-smoke.mjs`、`node --check scripts/mac/test-mac-formal-local-smoke.mjs`、`node scripts/mac/test-mac-formal-local-smoke.mjs --timeoutMs 30000`、`node scripts/mac/test-mac-script-help.mjs --script check-mac-formal-local-smoke.mjs --timeoutMs 10000 --boardSummary`、`LAN_DUAL_PASSWORD=formal-local-smoke-test-password node scripts/mac/check-mac-formal-local-smoke.mjs --json --host 127.0.0.1 --port 9 --timeoutMs 3000 --skipVideo --skipAudio --skipInputLog`、`git diff --check` 与触碰文件冲突标记扫描均通过；无密码打印、无真实 host 认证、无 call/input/inject。
遗留问题：
- 当前真实 Mac host 仍是旧 build；本轮没有重启 host，也没有跑需要用户密码的真实 browser smoke。
下一步建议：
- 只看到 `MacFormalLocalSmoke=` / `RerunFormalLocalSmoke=` 时，无人值守可先复制 `MacClientBrowserSelfTest=`；用户在场且准备真实连接密码时再复制 `MacClientPromptPasswordSmoke=`。
...
- 诊断详情会保留原始 `MacClientBrowserSelfTest=node scripts/mac/test-mac-client-browser-self-test-wrapper.mjs --boardSummary`，但 Windows 端不会运行 Mac 脚本。
- Windows 本机 Mac 提醒 watcher 增加专项覆盖：带非空 findings 的 self-test 摘要会提醒，干净 `warnings=none blockers=none` 命令清单不提醒。
修改文件：
- `apps/windows-client/app.js`
- `scripts/windows/test-windows-client-browser.mjs`
- `scripts/windows/test-mac-alert-watcher.mjs`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/windows/test-windows-client-browser.mjs --diagnosticsOnly --timeoutMs 45000` 先失败在缺少“Mac client 本地 browser 自测命令已提供”。
- 绿灯：实现后复跑同一命令通过。
- `node scripts/windows/test-mac-alert-watcher.mjs --timeoutMs 20000` 通过，覆盖 self-test findings 与 clean ignored。
- `node --check apps/windows-client/app.js`
- `node --check scripts/windows/test-windows-client-browser.mjs`
- `node --check scripts/windows/test-mac-alert-watcher.mjs`
- `git diff --check`
- 冲突标记扫描通过。
遗留问题：
- 真实 Mac -> Windows browser smoke 仍需要现场密码或 Windows 安全认证路径；本轮只消费无密自测入口，不做认证。
下一步建议：
- 后续 Mac formal smoke 若因真实 host 密码/LAN/授权失败但给出 `MacClientBrowserSelfTest=`，先让 Mac 跑本地 mock browser 自测排除页面退化，再回到真实 smoke。
是否改了协议：否；只补 Windows 侧诊断识别和测试。
是否需要另一端配合：不需要；Mac 端已提供同名摘要入口。
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-formal-e2e-status.mjs --timeoutMs 45000` 失败在旧 runtime build 缺结构化 `suggestedAction`。
- 绿灯：实现后复跑同一自测通过。
- 真实本机抽样：当前 `127.0.0.1:43770` 的 formal E2E 摘要已显示 `suggestedAction=restart-mac-host-safely actionCommands=MacHostStop->MacHostSafeStart-or-MacMaxFpsSafeStart->MacResumeStatus`。
遗留问题：
- 当前本机 Mac host 仍是旧 build；本轮没有重启它，因为安全启动需要用户在本机可见密码提示里授权。
下一步建议：
- 白天正式验收前，看到 Mac formal E2E 的 `suggestedAction=restart-mac-host-safely` 时，按摘要顺序先停旧本机 host，再由用户授权安全前台启动，最后跑 `MacResumeStatus` / `MacHostMedia` 确认 build 和媒体基线。
是否改了协议：否；只补 Mac formal E2E readiness 的只读建议字段和摘要文案。
...
日期：2026-06-19 继续推进
开发端：Mac Codex
本轮目标：让 Mac formal E2E readiness 也暴露稳定媒体基线入口。
完成内容：
- `check-mac-formal-e2e-status --json/--boardSummary` 现在输出 `commands.macHostMediaCommand` / `MacHostMedia=node scripts/mac/check-mac-host-readiness.mjs --host <host> --port <port> --checkBoard --probeMedia --probeMediaResourceSample --promptPassword --boardSummary`。
- 只看到 `MacFormalE2E=` 摘要时，也能直接让 Mac 端在正式 H.264/PCM 长测前刷新媒体基线，不必回翻 heartbeat、resume 或 unattended 摘要。
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-formal-e2e-status.mjs --timeoutMs 45000` 失败在 help 缺 `commands.macHostMediaCommand`。
- 绿灯：实现后复跑同一自测通过。
遗留问题：
- 当前本机 Mac host 仍是旧 build；本轮没有重启它，也没有跑需要密码的媒体基线探针。
下一步建议：
- 白天正式验收前，看到 `MacFormalE2E=` 后可先按 `suggestedAction` 处理旧 host build，再跑 `MacHostMedia` 刷新 H.264/PCM 媒体基线。
...
- JSON 新增 `commands.macResumeStatusCommand` / `suggestedAction.id/reason/commands`，摘要新增 `MacResumeStatus=` 和同名 `suggestedAction` 安全动作顺序。
- 这只是只读建议，不会自动停止旧 host、不会启动新 host、不会弹密码、不会认证 WebSocket，也不会发送 call/input/inject。
修改文件：
- `scripts/mac/check-mac-host-readiness.mjs`
- `scripts/mac/test-mac-host-readiness-board.mjs`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-host-readiness-board.mjs --timeoutMs 45000` 失败在 help 缺 `commands.macResumeStatusCommand`。
- 绿灯：实现后复跑同一自测通过。
遗留问题：
- 当前本机 Mac host 仍是旧 build；本轮没有重启它，因为安全启动需要用户在本机可见密码提示里授权。
下一步建议：
- 白天正式验收前，看到 Mac host readiness、resume 或 heartbeat 的 `suggestedAction=restart-mac-host-safely` 时，按摘要顺序先停旧本机 host，再由用户授权安全前台启动，最后跑 `MacResumeStatus` / `MacHostMedia` 确认 build 和媒体基线。
是否改了协议：否；只补 Mac host readiness 的只读建议字段和摘要文案。
是否需要另一端配合：本轮不需要；Windows 端可后续按需消费 `MacHostReadiness` 里的同名 suggestedAction。
...
- Windows 端或人工只看最新 `MacHeartbeat=` 时，也能直接复制媒体基线检查入口，不必先要求 Mac 跑 `MacResumeStatus=` 再翻出 `MacHostMedia=`。
- 该命令只作为安全提示输出，不自动运行；真正执行时本地可见提示输入密码，不把密码放进 argv、通讯板或日志，不认证、不发送 call/input/inject。
修改文件：
- `scripts/mac/check-mac-heartbeat.mjs`
- `scripts/mac/test-mac-heartbeat.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-heartbeat.mjs --timeoutMs 12000` 失败在 board summary 缺 `MacHostMedia=`。
- 绿灯：实现后复跑同一自测通过。
遗留问题：
- 本轮只补 heartbeat 摘要的媒体基线入口，没有重启 Mac host、请求密码、认证真实 host、发送 call/input 或执行 `inject`。
下一步建议：
- Windows 或人工只看到 `MacHeartbeat=` 时，可直接复制 `MacHostMedia=` 让 Mac 端刷新 H.264/PCM 媒体基线；如果要正式验收，仍按摘要里的安全启动、local smoke 和 formal E2E 流程执行。
...
- 普通输出和失败时的 NEXT 提示也会显示同一条 help 安全自检命令。
- 只拿到 `MacFormalLocalSmoke=` / `RerunFormalLocalSmoke=` 或本机短验收失败摘要时，也能直接要求 Mac 端跑统一 `--help/-h` 副作用防线，不必回翻 heartbeat、resume、readiness 或 formal E2E 摘要。
修改文件：
- `scripts/mac/check-mac-formal-local-smoke.mjs`
- `scripts/mac/test-mac-formal-local-smoke.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-formal-local-smoke.mjs --timeoutMs 20000` 失败在 help 缺 `commands.macScriptHelpCommand`。
- 绿灯：实现后复跑同一自测通过。
遗留问题：
- 本轮只补 Mac 本机短验收摘要的安全自检入口，没有请求密码、认证真实 host、发送 Agent Link Board call、发送 input 或执行 `inject`。
下一步建议：
- Windows 或人工只看到 `MacFormalLocalSmoke=` / `RerunFormalLocalSmoke=` 摘要时，可直接复制 `MacScriptHelp=` 让 Mac 端确认所有 Mac `.mjs` 脚本 help 路径无副作用；正式本机短验收仍需用户在 Mac 前台授权密码弹窗。
是否改了协议：否；只补 Mac formal local smoke 的安全命令标签。
是否需要另一端配合：本轮不需要；Windows 端可后续按需消费该标签。
...
- `--help/-h` 的机器可读字段说明补 `commands.macScriptHelp`，普通输出也会显示 `Mac script help:`。
- Windows 端或人工只看 `MacClientFormalSmoke=` / formal smoke 摘要时，也能要求 Mac 端跑统一 `--help/-h` 副作用防线，不必回翻 heartbeat、resume、readiness、formal checklist 或 discovery 摘要。
- 自测新增回归：help、preflight JSON/board summary、sendCall 摘要、dryRun、discover preflight、ensureClient preflight、discover sendCall、discover failure 和 no-password failure 都必须包含安全的 `MacScriptHelp=` 命令，且命令不含密码、call、input 或 inject 路径。
修改文件：
- `scripts/mac/run-mac-client-formal-smoke.mjs`
- `scripts/mac/test-mac-client-formal-smoke.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-client-formal-smoke.mjs --timeoutMs 20000` 失败在 help 缺 `commands.macScriptHelp`。
- 绿灯：实现后复跑同一自测通过。
遗留问题：
- 本轮只补 Mac formal smoke 摘要的安全自检入口，没有进行真实 Windows host 密码连接、反控授权或输入注入验收。
下一步建议：
- Windows 若只拿到 `MacClientFormalSmoke=` / `run-mac-client-formal-smoke --boardSummary`，也可复制 `MacScriptHelp=` 让 Mac 端快速确认所有 Mac `.mjs` 脚本 help 路径无副作用；正式 Mac 控 Windows 真连仍需用户在场时按安全认证/一次性授权流程走。
是否改了协议：否；只补 Mac client formal smoke 的安全命令标签。
是否需要另一端配合：本轮不需要；Windows 端可后续按需消费该标签。
...
- Windows 端或人工只看 `MacClientFormalChecklist=` / formal checklist 摘要时，也能要求 Mac 端跑统一 `--help/-h` 副作用防线，不必回翻 resume、heartbeat 或 client readiness 摘要。
- 自测新增回归：help、离线 JSON、允许离线 warnings、纯 board summary、人类可读 runPlan、mock ready shape 和 discover shape 都必须包含安全的 `MacScriptHelp=` 命令，且命令不含密码、call、input 或 inject 路径。
修改文件：
- `scripts/mac/check-mac-client-formal-status.mjs`
- `scripts/mac/test-mac-client-formal-status.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-client-formal-status.mjs --timeoutMs 12000` 失败在 help 缺 `runPlan.commands.macScriptHelp`。
- 绿灯：实现后复跑同一自测通过。
- `node --check scripts/mac/check-mac-client-formal-status.mjs`
- `node --check scripts/mac/test-mac-client-formal-status.mjs`
- `node scripts/mac/test-mac-client-formal-status.mjs --timeoutMs 12000`
- `node scripts/mac/test-mac-script-help.mjs --timeoutMs 10000 --boardSummary`
- 真实只读 formal checklist 摘要抽样：`Mac client formal Windows test:` 输出 `MacScriptHelp=node scripts/mac/test-mac-script-help.mjs --timeoutMs 10000 --boardSummary`，并明确不要在通讯板发送密码、不要执行 inject。
- `git diff --check` 与冲突标记扫描通过。
遗留问题：
- 本轮只补 Mac client formal checklist 摘要的安全自检入口，没有进行真实 Windows host 密码连接、反控授权或输入注入验收。
下一步建议：
- Windows 若只拿到 `MacClientFormalChecklist=`，也可复制 `MacScriptHelp=` 让 Mac 端快速确认所有 Mac `.mjs` 脚本 help 路径无副作用；正式 Mac 控 Windows 真连仍需用户在场时按安全认证/一次性授权流程走。
是否改了协议：否；只补 Mac client formal checklist 的安全命令标签。
是否需要另一端配合：本轮不需要；Windows 端可后续按需消费该标签。
...
日期：2026-06-19 继续推进
开发端：Mac Codex
本轮目标：让 Mac client readiness 也暴露 Mac 脚本 help 安全自检入口。
完成内容：
- `check-mac-client-readiness --json/--boardSummary` 现在输出 `commands.macScriptHelpCommand` / `MacScriptHelp=node scripts/mac/test-mac-script-help.mjs --timeoutMs 10000 --boardSummary`。
- Windows 端或人工只看 `MacClientDiagnostics=` / `check-mac-client-readiness` 摘要时，也能要求 Mac 端跑统一 `--help/-h` 副作用防线，不必回翻 resume、heartbeat、unattended 或 host readiness 摘要。
- 自测新增回归：help、离线 JSON、普通输出、一行 board summary、本地 Mac client server probe 和 mock Windows discovery probe 都必须包含安全的 `MacScriptHelp=` 命令，且命令不含密码、call、input 或 inject 路径。
修改文件：
- `scripts/mac/check-mac-client-readiness.mjs`
- `scripts/mac/test-mac-client-readiness.mjs`
- `docs/CURRENT_STATUS.md`
...
- `docs/ACTIVE_LOCKS.md`
验证方式：
- 红灯：`node scripts/mac/test-mac-client-readiness.mjs --timeoutMs 12000` 失败在 help 缺 `commands.macScriptHelpCommand`。
- 绿灯：实现后复跑同一自测通过。
...
- `node scripts/mac/test-mac-script-help.mjs --timeoutMs 10000 --boardSummary`
- 真实只读 Mac client readiness 摘要抽样：`Mac client readiness:` 输出 `MacScriptHelp=node scripts/mac/test-mac-script-help.mjs --timeoutMs 10000 --boardSummary`，并明确不要在通讯板发送密码。
- `git diff --check` 与冲突标记扫描通过。
遗留问题：
- 本轮只补 Mac client readiness 摘要的安全自检入口，没有进行真实 Windows host 密码连接或反控注入验收。
下一步建议：
- Windows 若只拿到 `MacClientDiagnostics=` 或 Mac client readiness 摘要，也可复制 `MacScriptHelp=` 让 Mac 端快速确认所有 Mac `.mjs` 脚本 help 路径无副作用；正式 Mac 控 Windows 真连仍需用户在场时按安全认证/一次性授权流程走。
是否改了协议：否；只补 Mac client readiness 的安全命令标签。
是否需要另一端配合：本轮不需要；Windows 端可后续按需消费该标签。
