...
启动器会优先使用 PowerShell 7 `pwsh`，找不到时再回退 Windows PowerShell。它会把日志写到 `.dev-lab/mac-alert-watcher.*.log`，并用 `.dev-lab/mac-alert-watcher.pid` 防止重复启动，适合把 Mac 窗口最小化后继续让 Windows 本机弹窗提醒授权、权限、502、blocked 或长时间无更新。
