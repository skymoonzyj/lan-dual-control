...
- Mac host 重启或部署后，先看 `/discovery.runtime`、Windows 控制端诊断条，或运行 `node scripts/mac/check-mac-displays.mjs --requireRuntime --expectBuildId <build-id>`，确认 `processId`、`startedAt`、`uptimeSeconds` 和 `buildId` 符合预期；刚重启时可加 `--maxRuntimeUptimeSeconds 120`，避免连到旧进程。
...
- Mac 控制 Windows 真机联调前，先运行 `node scripts/windows/check-windows-firewall.mjs --host 0.0.0.0 --port 43770 --requireOpen` 检查 Windows host 是否监听到局域网地址、LAN IP 是否可连以及 TCP 入站放行是否存在；该脚本默认只读，不会自动改系统防火墙。
