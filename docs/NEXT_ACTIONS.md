...
- 增加真实多显示器枚举和采集切换。
- 继续完善 `apps/mac-client` Mac 控制 Windows 原型：接真实 Windows host 验证、增加剪贴板/音频播放、打磨键盘映射和错误提示。
...
- Windows host 相关改动后运行 `scripts/windows/test-windows-host.ps1`，确认真实 JPEG 首帧、文本剪贴板和文件剪贴板接收未退化；涉及 Mac 反控链路时再运行 `node scripts/windows/test-mac-client-browser.mjs`，确认 Mac client 页面可显示 Windows 画面并收到 `input_ack`。
