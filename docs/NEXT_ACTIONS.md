...
- 日常启动真实 Mac host 优先用 `node scripts/mac/start-mac-host.mjs --promptPassword --requirePassword`；该入口默认 `inputMode=log`、打印 Windows 可连接的局域网地址、等待 `/discovery`，并跑只读 runtime/display 校验。直接 `swift run lan-dual-mac-host` 时源码默认也应保持 `log`，可用 `node scripts/mac/test-mac-host-defaults.mjs` 回归；需要真实注入时，必须有人在屏幕前确认安全后再显式加 `--inputMode inject`、`--injectInput` 或 `LAN_DUAL_INPUT_MODE=inject`。
