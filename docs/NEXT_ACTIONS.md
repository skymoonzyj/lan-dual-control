...
- 增加真实多显示器枚举和采集切换。
- 继续完善 `apps/mac-client` Mac 控制 Windows 原型：接真实 Windows PCM 音频播放，继续打磨键盘映射边界，后续可补最近连接清空/重命名等小体验。
