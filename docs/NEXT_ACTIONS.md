...
- Windows 被控端 FFmpeg gdigrab 过渡层普通启动已可协商 60Hz，当前本机 720p/60Hz 基线为 4 秒 230 帧、57.1 FPS、最大间隔 41 ms、帧年龄最大 1 ms；Windows Graphics Capture 支持预检已通过但尚未替换采集管线。下一步把采集层升级为 WGC 和正式编码管线时，至少要对照这条基线比较帧率、延迟、带宽和资源占用。
