...
- 增加真实多显示器枚举和采集切换。
- 继续完善 `apps/mac-client` Mac 控制 Windows 原型：用 Windows 默认 `node scripts/windows/test-mac-client-browser.mjs` 强校验文件剪贴板 `saveMode=clipboard`，接真实 Windows PCM 音频播放，打磨键盘映射和连接体验。
