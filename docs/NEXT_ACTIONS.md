...
- Windows 被控端 FFmpeg gdigrab 过渡层普通启动已可协商 60Hz，当前本机 720p/60Hz 基线为 4 秒 230 帧、57.1 FPS、最大间隔 41 ms、帧年龄最大 1 ms；观察脚本现在也能采样本机 Windows host 资源，`--resourceSampleTree true` 下的 60Hz 对照为 4 秒 198 帧、49.49 FPS、最大间隔 43 ms、帧年龄最大 1 ms、进程树 CPU 平均/峰值 4.5/5.4%、工作集峰值约 309.3 MiB。Windows Graphics Capture 支持预检已通过，且 `LAN_DUAL_WINDOWS_SCREEN_MODE=wgc` 显式入口会输出 `screen.wgc` 与 `wgcFallbackReason`；`LAN_DUAL_WINDOWS_WGC_HELPER` JSON 行 helper 接入点已落地，新增 Rust helper `apps/windows-wgc-helper`，本机 `--probe` 已能初始化 D3D11/WinRT Direct3D/GraphicsCaptureItem/frame pool/session，默认 capture 已能从 `Direct3D11CaptureFrame.Surface` 读回真实帧并用 WIC 编码 JPEG；`test-windows-wgc-helper.mjs` 已验证真实一帧 `2560x1440` JPEG 和 Node host mock helper 合同接入，收到 37 帧 `windows-wgc-helper-jpeg`。下一步优先在 Rust helper 内实现请求分辨率缩放、JPEG 质量/码率控制和连续帧 pacing，再用 `node scripts/windows/test-windows-wgc-helper.mjs`、`observe-windows-host-video --screenMode wgc --resourceSampleTree true --json` 和 Mac client 真实连接对照帧率、延迟、带宽和资源占用。短期继续用 `node scripts/windows/observe-windows-host-media.mjs --resourceSampleTree true --json` 顺序跑视频+音频统一报告，避免并发临时 host 干扰采集。本轮 `2026-06-13 13:00` 当前桌面会话里 FFmpeg `gdigrab` 曾出现 `Failed to capture image (error 5)` 并导致视频观察回退 mock，真实视频基线需在桌面捕获稳定后复测或由 WGC 替换。
