...
- W14 证据现在也进入上板链路：`test-windows-client-browser --boardSummary` 会输出 `W14NativeVideo=status=... transport=... mediaOwner=... videoFrames=... h264Frames=... pushed=... accepted=... dropped=... queueMs=... decoded=... presentFrames=... presenting=... lastStatus=...`。`post-w8-desktop-video-board` 可读取只含 `W14NativeVideo=` 的复制诊断，或把 `W8NativeVideo=` 与 `W14NativeVideo=` 合并为一条安全消息，并生成 `W14NativeGate=`：`presenting-ok` 代表 W14 receiver -> W8 MF/D3D11/HWND 已有接收、解码和呈现证据；`decode-next` 指向 MF decoder；`present-next` 指向 D3D11/HWND present；`receiver-next/receive-next/native-error-next` 分别指向 W14 连接/收帧/error。该入口仍拒绝 password/token/input_event，不认证、不请求密码、不发 input/inject。
