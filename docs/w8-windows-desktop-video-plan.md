...
- 该 MVP 已推进到 MF worker、D3D11 latest-frame texture 和 BGRA8 present texture 预接线，但还没有接真实 HWND swapchain/native renderer，不宣称已经完成最终原生画面绘制。
...
1. 把 BGRA8 present texture 接到真实 HWND swapchain 或 native renderer，窗口最小化 / 后台 / 切 app 时仍按实时队列丢旧保新。
