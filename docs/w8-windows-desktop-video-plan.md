...
- Windows 复测摘要现在会把 W8 原生视频状态压成 `W8NativeVideo=`：`test-windows-client-browser --boardSummary` 读取页面 snapshot 里的 `decoderSession/nativePresent/nativeSurface/windowSwapchain` 字段，输出 `status/present/presentFrames/decoded/presenting/presentGap/output/surface/copy/handoff/swapchain/streamChange/deviceLost/errors`；其中 `presenting=yes|no` 和 `presentGap=<decoded-presentFrames>` 用来直接判断真实长跑时原生窗口 Present 是否跟上解码。`Run-WinClientRetest-And-Post.cmd` 会把这条 W8 摘要单独发布到 Agent Link Board，和旧 `W2W3Retest=` 分开，避免污染 W2 H.264 对照诊断。
...
- Windows 复测摘要测试必须覆盖：真实复测输出里的 `W8NativeVideo=` 会被 `post-w2w3-retest-board` 安全提取、单独上板，并且不会把 W8 的 `decoded/present` 字段混进 `W2H264BoardDiagnosis=`；同一摘要必须包含 `presenting=yes|no` 与 `presentGap=<n>`，防止长跑时只看到静态 `presentFrames/decoded` 而无法快速判断原生呈现是否脱节。
