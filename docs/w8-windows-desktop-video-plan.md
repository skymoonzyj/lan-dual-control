...
- 该 MVP 已推进到 MF worker、D3D11 latest-frame texture、BGRA8 present texture、真实 HWND swapchain 预检、BGRA8 latest-frame -> HWND swapchain `Present`、NV12 -> BGRA8 `VideoProcessorBlt` -> HWND swapchain `Present`、NV12 窗口 client resize 后的 `ResizeBuffers` / present texture 重建、stream-change 后输出重选 / native surface 重建，以及 device-lost 后 native surface / present target 重建。后续要让它成为最终体验路径，还需要真实 Mac 长时间观感验证。
