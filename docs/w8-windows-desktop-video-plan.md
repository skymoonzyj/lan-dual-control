...
- Windows 复测摘要测试必须覆盖：真实复测输出里的 `W8NativeVideo=` 会被 `post-w2w3-retest-board` 安全提取、单独上板，并且不会把 W8 的 `decoded/present` 字段混进 `W2H264BoardDiagnosis=`。
