...
function isWindowsPushCriticalStatus(status, note) {
  const statusText = normalizedText(status).toLowerCase();
  if (/\b(pushing-soon|pushing|rebasing|merging|resolving-conflicts|committing)\b/.test(statusText)) return true;
  if (/^(online|idle|pushed|done|complete|completed|ok)$/.test(statusText)) return false;
  const combined = `${statusText} ${normalizedText(note)}`.toLowerCase();
  return /\b(pushing-soon|pushing|rebasing|merging|resolving-conflicts|committing)\b/.test(combined)
    ...
    || /(?:提交|推送|变基|合并|解决冲突)[^。；;\n]{0,40}(?:准备|即将|正在|开始)/.test(combined);
}
