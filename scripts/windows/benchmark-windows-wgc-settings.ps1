...
param(
  ...
  [switch] $RepeatLastFrame,
  [ValidateSet("full", "signal")]
  [string] $RepeatLastFrameMode = "full",
  [switch] $H264Bridge,
  [ValidateSet("jpeg", "raw-bgra", "nv12")]
  [string] $H264Source = "jpeg",
  [string] $H264Encoder = "",
...
if ($Help) {
  Write-Output @"
Usage:
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts\windows\benchmark-windows-wgc-settings.ps1 [options]

Common examples:
  # One-line Agent Link Board summary, safe to paste.
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts\windows\benchmark-windows-wgc-settings.ps1 -Profile 60:20000:balanced -DurationMs 1800 -BoardSummary

  # Machine-readable report.
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts\windows\benchmark-windows-wgc-settings.ps1 -Profile 60:20000:balanced -Json

  # H.264/NV12 bridge sample with repeat-last-frame pacing.
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts\windows\benchmark-windows-wgc-settings.ps1 -Profile 60:20000:balanced -H264Bridge -H264Source nv12 -H264Encoder h264_nvenc -RepeatLastFrame -BoardSummary

Options:
  -HostName <host>              Local benchmark host bind target. Default: 127.0.0.1.
  -Port <port>                  First temporary host port. Default: 43782.
  -Profile <fps:kbps:preset[]>  Benchmark profile(s). Default: 30:10000:balanced,60:20000:balanced,120:40000:sharp.
  -Helper <path>                WGC helper executable path.
  -Width <px> -Height <px>      Requested video size. Default: 1280x720.
  -DurationMs <ms>              Per profile observation window. Default: 2500.
  -TimeoutMs <ms>               Per child command timeout. Default: 45000.
  -MinFrames <n>                Minimum frames for each profile.
  ...
  -H264Bridge                   Request H.264 and enable WGC helper -> FFmpeg bridge.
  -H264Source <source>          jpeg | raw-bgra | nv12.
  -H264Encoder <name>           H.264 encoder, for example h264_nvenc.
  -MotionStimulus               Open a temporary animated window before probing.
  ...
  -SkipBuild                    Do not build the Rust helper before benchmarking.
...
$nodeArgs = @(
  ...
  "--repeatLastFrame", (ConvertTo-NodeBool $RepeatLastFrame),
  "--repeatLastFrameMode", $RepeatLastFrameMode,
  "--h264Bridge", (ConvertTo-NodeBool $H264Bridge),
