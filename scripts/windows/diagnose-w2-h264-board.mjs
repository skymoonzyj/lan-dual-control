...
function extractWindowsEvidence(entries) {
  ...
  for (const entry of ordered) {
    ...
    const evidence = {
      ...
      canvas: tokens.canvas !== undefined ? String(tokens.canvas) : undefined,
      image: tokens.image !== undefined ? String(tokens.image) : undefined,
      updatedAt: entry.at,
    };
    return evidence;
  }
...
function boardSummaryFor(payload) {
  ...
  return [
    ...
    `surface=canvas:${compactNumber(windows.canvas)} image:${compactNumber(windows.image)}`,
...
function textFor(payload) {
  ...
  return [
    ...
    `next: ${payload.next}`,
    `windows: recv=${compactNumber(windows.recv)} key=${compactNumber(windows.key)} sps=${compactNumber(windows.sps)} pps=${compactNumber(windows.pps)} idr=${compactNumber(windows.idr)} decoded=${compactNumber(windows.decoded)} lastNal=${compactNumber(windows.lastNal)} needsKeyframe=${compactNumber(windows.needsKeyframe)} canvas=${compactNumber(windows.canvas)} image=${compactNumber(windows.image)} reason=${compactNumber(windows.reason)}`,
