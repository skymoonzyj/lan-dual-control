...
async function checkDryRunDoesNotPost(args) {
  await withFakeBoard(async (board) => {
    ...
    assertIncludes(payload.w8NativeVideoLine, "webDecode=native-main-surface", "dry-run W8 native video line");
    assertIncludes(payload.w8NativeVideoLine, "webBypass=24", "dry-run W8 native video line");
    ...
    assertIncludes(payload.w8NativeGateSummary, "pushed=192", "dry-run W8 native gate");
    ...
    assertIncludes(payload.w8ArrivalBacklogSummary, "localAvgMs=16", "dry-run W8 arrival backlog");
    assertIncludes(payload.w8ArrivalBacklogSummary, "localMaxMs=9100", "dry-run W8 arrival backlog");
    assertIncludes(payload.w8ArrivalBacklogSummary, "remoteMediaAvgMs=17", "dry-run W8 arrival backlog");
    assertIncludes(payload.w8ArrivalBacklogSummary, "remoteMediaMaxMs=21", "dry-run W8 arrival backlog");
...
async function checkSendRetestAndDiagnosis(args) {
  await withFakeBoard(async (board) => {
    ...
    assertIncludes(board.messages[1].text, "webDecode=native-main-surface", "posted W8 native video message");
    assertIncludes(board.messages[1].text, "webBypass=24", "posted W8 native video message");
    assertIncludes(board.messages[1].text, "pushed=192", "posted W8 native video message");
    assertIncludes(board.messages[1].text, "submitted=190", "posted W8 native video message");
    assertIncludes(board.messages[1].text, "decoderGap=2", "posted W8 native video message");
