...
const retestLine = "W2W3Retest=video=real-h264 surface=none h264=status=rendering decoded=3722 skippedDelta=0 needsKeyframe=no queue=4 queueMs=190 staleDrops=125 reason=live-backlog-keyframe-request recv=3722 key=87 sps=87 pps=87 idr=87 lastNal=7/8/5 liveBacklogRequests=42 maxGapMs=9100 visibilityRecovery=2, audio=queue 100 ms";
...
async function checkSuccessfulRetestPostsRetestAndDiagnosis(args) {
  await withFakeBoard(async (board) => {
    ...
    assertIncludes(board.messages[1].text, "queueMs=190", "posted W8 native video message");
