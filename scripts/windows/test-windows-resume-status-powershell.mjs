...
async function checkWrapperHelp(args) {
  ...
  assertIncludes(output, "Start-Windows-Desktop-Control-Mac.cmd", "PowerShell wrapper help");
  assertIncludes(output, "Build-Windows-Desktop-Control-Mac.cmd", "PowerShell wrapper help");
...
async function checkMockJson(args) {
  await withMockHost(async (port) => {
    ...
    assert(payload.commands?.windowsDesktopEntry?.nativeVideoPostCommand === windowsDesktopNativeVideoPostCommand, "PowerShell mock JSON should expose Windows desktop native video post command");
...
async function checkBoardSummary(args) {
  await withMockHost(async (port) => {
    ...
    assertIncludes(output, `WindowsDesktopEntry=${windowsDesktopEntrySummary}.`, "PowerShell board summary Windows desktop entry");
    assertIncludes(output, "web=diagnostic-only", "PowerShell board summary Windows desktop entry should mark Web diagnostic-only");
