...
async function checkHelp(args) {
  for (const flag of ["--help", "-h"]) {
    ...
    assertIncludes(result.stdout, "--sendManualUxAck", `help ${flag}`);
    ...
    assertIncludes(result.stdout, "Start-Windows-Desktop-Control-Mac.cmd", `help ${flag}`);
    assertIncludes(result.stdout, "Build-Windows-Desktop-Control-Mac.cmd", `help ${flag}`);
    ...
    assertIncludes(result.stdout, "MacManualUxStatus=", `help ${flag}`);
    assertIncludes(result.stdout, "check-mac-manual-ux-status.mjs --boardSummary", `help ${flag}`);
...
async function checkMockJson(args) {
  await withMockHost(async (port) => {
    ...
    assert(payload.commands?.windowsDesktopEntry?.nativeVideoPostCommand === windowsDesktopNativeVideoPostCommand, "mock JSON should expose Windows desktop native video post command");
    ...
    assert(payload.commands?.windowsClientRetestAndPostUserEntryCommand === windowsClientRetestAndPostUserEntryCommand, "mock JSON should include Windows client retest-and-post root entry");
    assert(payload.w2VisibilityRetest?.status === "pending-user-retest", "mock JSON should expose W2 visibility retest status");
...
async function checkBoardSummary(args) {
  await withMockHost(async (port) => {
    ...
    assertIncludes(result.stdout, `WindowsDesktopEntry=${windowsDesktopEntrySummary}.`, "board summary Windows desktop entry");
    assertIncludes(result.stdout, "web=diagnostic-only", "board summary Windows desktop entry should mark Web diagnostic-only");
...
async function checkWindowsClientRetestRootEntry(args) {
  ...
  let andPostContent = "";
  try {
    andPostContent = await readFile(andPostEntryPath, "utf8");
  } catch (error) {
    assert(false, `missing Windows client retest-and-post root entry ${windowsClientRetestAndPostUserEntryCommand}: ${error.message}`);
  }

  assertIncludes(andPostContent, "run-winclient-retest-and-post.mjs", windowsClientRetestAndPostUserEntryCommand);
  assertIncludes(andPostContent, "%*", windowsClientRetestAndPostUserEntryCommand);
  assertNotIncludes(andPostContent, "--password", windowsClientRetestAndPostUserEntryCommand);
  assertNotIncludes(andPostContent, "LAN_DUAL_PASSWORD", windowsClientRetestAndPostUserEntryCommand);
  assertNotIncludes(andPostContent, "test-password", windowsClientRetestAndPostUserEntryCommand);
  assertNotIncludes(andPostContent, "secret", windowsClientRetestAndPostUserEntryCommand);
  assertNotIncludes(andPostContent, "token", windowsClientRetestAndPostUserEntryCommand);
...
async function checkBoardMacHeartbeatHealthExtraction(args) {
  ...
  await withMockHost(async (port) => {
    ...
    await withMockLinkBoard(async (board) => {
      ...
      assert(payload.board?.macCodexHealth?.found === true, "current MacCodexHealth should be found");
      assert(payload.board.macCodexHealth.status === "ok", "current MacCodexHealth status mismatch");
      assert(payload.board.macCodexHealth.reason === "ok", "current MacCodexHealth reason mismatch");
      assert(payload.board.macCodexHealth.codexStatus === "coding", "current MacCodexHealth codexStatus mismatch");
      assert(payload.board.macCodexHealth.updatedAt === currentCodexUpdatedAt, "current MacCodexHealth updatedAt mismatch");
      assert(payload.board.macCodexHealth.ageMs === 15000, "current MacCodexHealth ageMs mismatch");
      assert(payload.board.macCodexHealth.thresholdMs === 300000, "current MacCodexHealth thresholdMs mismatch");
      assert(payload.board.macCodexHealth.checkedAt === currentCodexCheckedAt, "current MacCodexHealth checkedAt mismatch");
      ...
      assertIncludes(payload.boardSummary, `MacCodexHealth=ok checkedAt=${currentCodexCheckedAt} reason=ok codexStatus=coding updatedAt=${currentCodexUpdatedAt} ageMs=15000 thresholdMs=300000 blockers=none warnings=none.`, "current MacCodexHealth board summary");
    ...
    await withMockLinkBoard(async (board) => {
      ...
      assertIncludes(result.stdout, `MacCodexHealth=blocked checkedAt=${blockedCheckedAt} reason=mac-codex-stale blockers=mac-codex-stale warnings=none.`, "MacCodexHealth blocked board summary");
    ...
    await withMockLinkBoard(async (board) => {
      ...
      assertIncludes(result.stdout, `MacCodexHealth=blocked checkedAt=${blockedCheckedAt} reason=mac-codex-stale blockers=mac-codex-stale warnings=none`, "MacCodexHealth human output");
...
async function checkBoardMacHostSafeStartExtraction(args) {
  ...
  const macInputSafetyUserNoticeSummary = "goal=verify-real-mac-input-safe-event-set action=watch-mac-screen-and-be-ready-to-take-over boundary=safe-event-set-only-no-click-delete-shortcuts-return-log duration=2-3-minutes";
  const macInputSafetyStatusBoardText = `MacInputSafetyStatus=ready reason=log-mode-permissions-ok host=online inputMode=log permissions=ok realInput=blocked-until-user-watching required=--confirmUserWatching eventSet=safe blockers=none warnings=none UserNoticeGoal=verify-real-mac-input-safe-event-set UserNoticeAction=watch-mac-screen-and-be-ready-to-take-over UserNoticeBoundary=safe-event-set-only-no-click-delete-shortcuts-return-log UserNoticeDuration=2-3-minutes. MacInputSafetyPlan=${macInputSafetyPlanCommand}. MacInputLogSmoke=node scripts/mac/smoke-mac-input-log.mjs --host 127.0.0.1 --port 43770 --promptPassword --boardSummary. Safety=no-password,no-auth,no-input-events,no-inject.`;
  ...
  const standaloneDiagnosticsEvidence = "Mac client diagnostics refreshed: MacClientDiagnosticsOk 已复验通过";
  ...
  await withMockHost(async (port) => {
    const boardState = {
      ...
      events: [
        ...
        },
        {
          type: "message",
          from: "Mac Codex",
          text: standaloneDiagnosticsEvidence,
        },
        {
          type: "message",
          from: "Mac Codex",
          text: "Mac client diagnostics failed: MacClientDiagnosticsOk failed blockers=client-diagnostics",
        },
        {
          type: "message",
          from: "Mac Codex",
        ...
        },
        {
          type: "message",
          from: "Mac Codex",
          text: "UserNoticeGoal=verify-real-mac-input-safe-event-set UserNoticeAction=watch-mac-screen-and-be-ready-to-take-over UserNoticeBoundary=safe-event-set-only-no-click-delete-shortcuts-return-log UserNoticeDuration=password=secret-value",
        },
        {
          type: "message",
          from: "Mac Codex",
          text: "UserNoticeGoal=verify-real-mac-input-safe-event-set UserNoticeAction=send-input-event-automatically UserNoticeBoundary=safe-event-set-only-no-click-delete-shortcuts-return-log UserNoticeDuration=2-3-minutes",
        },
        {
          type: "message",
          from: "Mac Codex",
    ...
    await withMockLinkBoard(async (board) => {
      ...
      assert(payload.board?.macClientManualChecklist?.found === true, "MacClientManualChecklist should be found in board state");
      ...
      assert(payload.board?.macRemoteAudio?.found === true, "Mac remote audio summary should be found in board state");
      assert(payload.board.macRemoteAudio.summary === macRemoteAudioSummary, "Mac remote audio summary mismatch");
      assert(payload.board.macRemoteAudio.source === "api-state", "Mac remote audio summary should come from /api/state");
      assert(payload.board.macRemoteAudio.capture === "system-pcm-does-not-mute-local", "Mac remote audio capture mismatch");
      assert(payload.board.macRemoteAudio.remoteOnlyOptions?.includes("manual-mute-restore"), "Mac remote audio should include manual mute option");
      assert(payload.board.macRemoteAudio.remoteOnlyOptions?.includes("virtual-output-device"), "Mac remote audio should include virtual output option");
      assert(payload.board.macRemoteAudio.remoteOnlyOptions?.includes("product-toggle"), "Mac remote audio should include product toggle option");
      assert(payload.board.macRemoteAudio.recommended === "product-toggle-with-explicit-consent", "Mac remote audio recommended option mismatch");
      assert(payload.board.macRemoteAudio.safety?.includes("no-volume-change"), "Mac remote audio should include no-volume-change safety");
      assert(payload.board.macRemoteAudio.safety?.includes("no password/input/inject"), "Mac remote audio should include no password/input/inject safety");
      assert(payload.board.macRemoteAudio.rejectedCount >= 2, "unsafe Mac remote audio summaries should be rejected");
      ...
      assert(payload.board.macInputSafetyPlan.rejectedCount >= 2, "unsafe or non-board MacInputSafetyPlan should be rejected");
      assert(payload.board?.macManualUxStatus?.found === true, "MacManualUxStatus should be found in board state");
      ...
      assert(payload.board.macManualUx.gate === "wait-windows-codex-push", "MacManualUx gate mismatch");
      ...
      assert(payload.board.macManualUx.reconfirmCommandPresent === true, "MacManualUx reconfirm command should be present");
      assert(payload.board.macManualUx.manualUxReconfirmCommand === macManualUxReconfirmCommand, "MacManualUx reconfirm command mismatch");
      assert(payload.board.macManualUx.rejectedCount >= 3, "unsafe MacManualUx summaries should be rejected");
      assert(payload.board?.macManualUxAck?.status === "blocked", "timeout MacManualUx should block Windows manual UX ack");
      assert(payload.board.macManualUxAck.summary === macManualUxAckTimeoutSummary, "timeout MacManualUx ack summary mismatch");
      assert((payload.board.macManualUxAck.command || "") === "", "timeout MacManualUx should not build an ack send command");
      ...
      assert(payload.board.macInputSafety.required === "--confirmUserWatching", "Mac input safety required flag mismatch");
      assert(payload.board.macInputSafety.eventSet === "safe", "Mac input safety event set mismatch");
      assert(payload.board.macInputSafety.safety?.includes("no-password"), "Mac input safety should include no-password safety");
      assert(payload.board.macInputSafety.safety?.includes("no-input-events"), "Mac input safety should include no-input-events safety");
      assert(payload.board.macInputSafety.safety?.includes("no-inject"), "Mac input safety should include no-inject safety");
      assert(payload.board.macInputSafety.rejectedCount >= 2, "unsafe Mac input safety summaries should be rejected");
      assert(payload.board?.macInputSafetyStatus?.found === true, "MacInputSafetyStatus should be found in board state");
      assert(payload.board.macInputSafetyStatus.summary === macInputSafetyStatusSummary, "MacInputSafetyStatus summary mismatch");
      assert(payload.board.macInputSafetyStatus.status === "ready", "MacInputSafetyStatus status mismatch");
      assert(payload.board.macInputSafetyStatus.reason === "log-mode-permissions-ok", "MacInputSafetyStatus reason mismatch");
      assert(payload.board.macInputSafetyStatus.inputMode === "log", "MacInputSafetyStatus inputMode mismatch");
      assert(payload.board.macInputSafetyStatus.realInput === "blocked-until-user-watching", "MacInputSafetyStatus realInput mismatch");
      assert(payload.board.macInputSafetyStatus.eventSet === "safe", "MacInputSafetyStatus event set mismatch");
      assert(payload.board.macInputSafetyStatus.safety?.includes("no-auth"), "MacInputSafetyStatus should include no-auth safety");
      assert(payload.board.macInputSafetyStatus.safety?.includes("no-password"), "MacInputSafetyStatus should include no-password safety");
      assert(payload.board.macInputSafetyStatus.rejectedCount >= 2, "unsafe MacInputSafetyStatus summaries should be rejected");
      assert(payload.board?.macInputSafetyUserNotice?.found === true, "MacInputSafety UserNotice should be found in board state");
      assert(payload.board.macInputSafetyUserNotice.summary === macInputSafetyUserNoticeSummary, "MacInputSafety UserNotice summary mismatch");
      assert(payload.board.macInputSafetyUserNotice.goal === "verify-real-mac-input-safe-event-set", "MacInputSafety UserNotice goal mismatch");
      assert(payload.board.macInputSafetyUserNotice.action === "watch-mac-screen-and-be-ready-to-take-over", "MacInputSafety UserNotice action mismatch");
      assert(payload.board.macInputSafetyUserNotice.boundary === "safe-event-set-only-no-click-delete-shortcuts-return-log", "MacInputSafety UserNotice boundary mismatch");
      assert(payload.board.macInputSafetyUserNotice.duration === "2-3-minutes", "MacInputSafety UserNotice duration mismatch");
      assert(payload.board.macInputSafetyUserNotice.rejectedCount >= 2, "unsafe MacInputSafety UserNotice summaries should be rejected");
      ...
      assert(payload.board.macEvidence.summary === "MacHostMediaOk,MacFormalLocalSmokeOk,MacClientPageOnline,MacClientDiagnosticsOk", "Mac positive evidence summary mismatch");
      ...
      assert(payload.board.macEvidence.tokens.includes("MacClientDiagnosticsOk"), "standalone MacClientDiagnosticsOk evidence should be captured");
      ...
      assertIncludes(payload.boardSummary, `MacManualUxReconfirm=${macManualUxReconfirmCommand}.`, "MacManualUx reconfirm JSON board summary");
      assertIncludes(payload.boardSummary, `MacManualUxAck=${macManualUxAckTimeoutSummary}.`, "MacManualUx timeout ack JSON board summary");
      assertIncludes(payload.boardSummary, `MacInputSafety=${macInputSafetySummary}.`, "Mac input safety JSON board summary");
      assertIncludes(payload.boardSummary, `MacInputSafetyStatus=${macInputSafetyStatusSummary}.`, "MacInputSafetyStatus JSON board summary");
      assertIncludes(payload.boardSummary, `MacInputSafetyUserNotice=${macInputSafetyUserNoticeSummary}.`, "MacInputSafety UserNotice JSON board summary");
      ...
      assertIncludes(payload.boardSummary, "MacEvidence=MacHostMediaOk,MacFormalLocalSmokeOk,MacClientPageOnline,MacClientDiagnosticsOk.", "Mac positive evidence JSON board summary");
    ...
    await withMockLinkBoard(async (board) => {
      ...
      assertIncludes(result.stdout, `MacManualUxReconfirm=${macManualUxReconfirmCommand}.`, "MacManualUx reconfirm board summary");
      assertIncludes(result.stdout, `MacManualUxAck=${macManualUxAckTimeoutSummary}.`, "MacManualUx timeout ack board summary");
      assertIncludes(result.stdout, `MacInputSafety=${macInputSafetySummary}.`, "Mac input safety board summary");
      assertIncludes(result.stdout, `MacInputSafetyStatus=${macInputSafetyStatusSummary}.`, "MacInputSafetyStatus board summary");
      assertIncludes(result.stdout, `MacInputSafetyUserNotice=${macInputSafetyUserNoticeSummary}.`, "MacInputSafety UserNotice board summary");
      ...
      assertIncludes(result.stdout, "MacEvidence=MacHostMediaOk,MacFormalLocalSmokeOk,MacClientPageOnline,MacClientDiagnosticsOk.", "Mac positive evidence board summary");
    ...
    await withMockLinkBoard(async (board) => {
      ...
      assertIncludes(result.stdout, `MacManualUxReconfirm=${macManualUxReconfirmCommand}`, "MacManualUx reconfirm human output");
      assertIncludes(result.stdout, `MacInputSafety=${macInputSafetySummary}`, "Mac input safety human output");
      assertIncludes(result.stdout, `MacInputSafetyStatus=${macInputSafetyStatusSummary}`, "MacInputSafetyStatus human output");
      assertIncludes(result.stdout, `MacInputSafetyUserNotice=${macInputSafetyUserNoticeSummary}`, "MacInputSafety UserNotice human output");
      ...
      assertIncludes(result.stdout, "MacEvidence=MacHostMediaOk,MacFormalLocalSmokeOk,MacClientPageOnline,MacClientDiagnosticsOk", "Mac positive evidence human output");
...
async function checkSendManualUxAck(args) {
  ...
  await withMockHost(async (port) => {
    await withMockLinkBoard(async (board) => {
      ...
      assertIncludes(board.messages[0].text, "不请求密码", "sent ManualUxAck");
      assertIncludes(board.messages[0].text, "不发送 input/inject", "sent ManualUxAck");
      assertNotIncludes(JSON.stringify(board.messages), "secret-value", "sent ManualUxAck");
      assertNotIncludes(JSON.stringify(board.messages), "--password", "sent ManualUxAck");
      console.log("[OK] Windows resume status can send a secret-free ManualUxAck");
    }, {
...
async function checkSendManualUxAckBlockedByGate(args) {
  ...
  await withMockHost(async (port) => {
    await withMockLinkBoard(async (board) => {
      ...
      assert(result.exitCode !== 0, "sendManualUxAck should fail while MacManualUxGate is active");
      const payload = JSON.parse(result.stdout);
      assert(payload.board?.macManualUx?.gate === "wait-windows-codex-push", "gated MacManualUx should be parsed");
      assert(payload.board?.macManualUxAck?.status === "blocked", "gated MacManualUx should block sendManualUxAck");
      assert(payload.board.macManualUxAck.reason === "mac-manual-ux-gated", "gated MacManualUx block reason mismatch");
      assert(payload.sentManualUxAck?.requested === true, "gated sendManualUxAck should be requested");
      assert(payload.sentManualUxAck?.ok === false, "gated sendManualUxAck should fail");
      assert(board.messages.length === 0, `gated ManualUxAck should not post a board message, got ${board.messages.length}`);
      console.log("[OK] Windows resume status blocks ManualUxAck while MacManualUxGate is active");
    }, {
      currentCall: manualUxCallForWindows(),
      statuses: {
        "Mac Codex": {
...
async function checkSendManualUxAckTimeout(args) {
  const macManualUxChecklist = "connection/video/audio/clipboard/file/window/fullscreen/original/copy-diagnostics";
  const macManualUxTimeoutText = `MacManualUx=status=calling ManualUxChecklist=${macManualUxChecklist} ManualUxLabels=连接/画面/声音/文本剪贴板/文件剪贴板/窗口/全屏/原画/复制诊断 Signals=manualUxCallInProgress Target=192.168.31.122:43770 TargetSource=mac-host-discovery Next=ReconfirmManualUxCall Safety=no-password,no-input-inject NoFormalE2ERerun=true ManualUxCall=timeout warnings=manual-ux-call-timeout`;
  await withMockHost(async (port) => {
    await withMockLinkBoard(async (board) => {
      const result = await run([
        "--discover",
      ...
      assert(result.exitCode !== 0, "sendManualUxAck should fail when manual UX call timed out");
      const payload = JSON.parse(result.stdout);
      assert(payload.board?.macManualUxAck?.status === "blocked", "timeout MacManualUx should block sendManualUxAck");
      assert(payload.sentManualUxAck?.requested === true, "timeout sendManualUxAck should be requested");
      assert(payload.sentManualUxAck?.ok === false, "timeout sendManualUxAck should fail");
      assert(payload.failedChecks?.some((check) => check.name === "sendManualUxAck"), "ManualUxAck refusal should be named");
      assert(board.messages.length === 0, `timeout ManualUxAck should not post a board message, got ${board.messages.length}`);
      assertNotIncludes(result.stdout + result.stderr, "secret-value", "timeout ManualUxAck");
      console.log("[OK] Windows resume status refuses ManualUxAck after timeout");
    }, {
      currentCall: manualUxCallForWindows(),
      statuses: {
        "Mac Codex": {
