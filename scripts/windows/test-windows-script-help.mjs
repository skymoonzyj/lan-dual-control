...
function printHelp() {
  console.log(`Usage:
  node scripts/windows/test-windows-script-help.mjs [options]
...
Description:
  Verifies every scripts/windows/*.mjs utility supports pure --help and -h output.
  A passing check means the command exits 0, prints Usage/Options-style help, and
  returns quickly without starting services, probes, browsers, or input helpers.
...
async function main() {
  ...
  if (args.json) {
    ...
    console.log(summary.ok
      ? `[OK] Windows script help coverage passed: ${summary.commandsChecked} commands`
      : `[FAIL] Windows script help coverage failed: ${failed.length} command(s)`);
  }
